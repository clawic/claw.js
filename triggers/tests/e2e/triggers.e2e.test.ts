...
import { buildTriggersApp, TriggersClient } from "@clawjs/triggers";

test("triggers CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "triggers-e2e-"));
  const dbPath = path.join(tmpDir, "triggers.sqlite");
  ...
  try {
    ...
    const client = new TriggersClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "triggers");
