...
if (!group || argv.includes("--help")) {
  process.stdout.write([
    ...
    "  discord serve [--port N] [--secret TOKEN]",
    "  discord accounts add --name N [--credentials-ref SECRET]",
    "  discord accounts list",
    "  discord accounts remove --id ID",
    "  discord routing assign --account ID --target T --agent A",
    "  discord routing list [--account ID] [--agent A]",
    "  discord send --account ID --target T --content TEXT",
    "  discord messages [--account ID] [--target T] [--direction inbound|outbound]",
  ].join("\n") + "\n");
  process.exit(0);
}
async function main() {
  if (group === "serve") {
    const { app } = buildDiscordApp({ config: { ...(flags.port ? { port: Number(flags.port) } : {}), ...(flags.secret ? { sharedSecret: flags.secret } : {}) } });
    const addr = await app.listen({ host: flags.host ?? "127.0.0.1", port: flags.port ? Number(flags.port) : 4702 });
    process.stdout.write(`${addr}\n`); return;
  }
  const baseUrl = flags.url ?? "http://127.0.0.1:4702"; const token = flags.token ?? flags.secret ?? "";
  const client = new ChannelApiClient({ channel: "discord", baseUrl, token });
