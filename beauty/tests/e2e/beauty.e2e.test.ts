...
import { buildBeautyApp, BeautyClient } from "@clawjs/beauty";

test("beauty CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "beauty-e2e-"));
  const dbPath = path.join(tmpDir, "beauty.sqlite");
  ...
  try {
    ...
    const client = new BeautyClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "beauty");
