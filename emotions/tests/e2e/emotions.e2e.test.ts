...
import { buildEmotionsApp, EmotionsClient } from "@clawjs/emotions";
...
test("emotions CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new EmotionsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "emotions");
