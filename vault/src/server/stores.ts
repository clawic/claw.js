...
// and exposes a typed CRUD surface used by the Vault resolver and HTTP
...
export class VaultMetaStore {
  ...
  load(tenantId: string): VaultMetaSnapshot | undefined {
    const row = this.db
      .prepare("SELECT snapshot_json FROM vault_meta WHERE tenant_id = ?")
      .get(tenantId) as { snapshot_json: string } | undefined;
    return row ? deserializeVaultMeta(row.snapshot_json) : undefined;
  }

  save(tenantId: string, meta: VaultMetaSnapshot): void {
    const json = serializeVaultMeta(meta);
    const ts = nowIso();
    this.db
      .prepare(
        "INSERT INTO vault_meta (tenant_id, snapshot_json, updated_at) VALUES (?, ?, ?) " +
...
// ---------- Vault container store ----------

export class VaultContainerStore {
  constructor(private readonly db: SqliteDb) {}
  ...
  create(input: {
    ...
    this.db
      .prepare(
        "INSERT INTO vaults (id, tenant_id, name, icon, color, sort_order, trashed_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NULL, ?, ?)",
      )
      .run(row.id, row.tenant_id, row.name, row.icon, row.color, row.sort_order, row.created_at, row.updated_at);
    return row;
  }

  list(tenantId: string, includeTrashed = false): VaultContainerRow[] {
    const sql = includeTrashed
      ? "SELECT * FROM vaults WHERE tenant_id = ? ORDER BY sort_order ASC, created_at ASC"
      : "SELECT * FROM vaults WHERE tenant_id = ? AND trashed_at IS NULL ORDER BY sort_order ASC, created_at ASC";
    return this.db.prepare(sql).all(tenantId) as VaultContainerRow[];
  }

  rename(id: string, name: string): VaultContainerRow | undefined {
    this.db.prepare("UPDATE vaults SET name = ?, updated_at = ? WHERE id = ?").run(name, nowIso(), id);
    return this.db.prepare("SELECT * FROM vaults WHERE id = ?").get(id) as VaultContainerRow | undefined;
  }

  trash(id: string): void {
    this.db.prepare("UPDATE vaults SET trashed_at = ?, updated_at = ? WHERE id = ?").run(nowIso(), nowIso(), id);
  }
}
...
export class SecretStore {
    ...
    try {
      const tx = this.db.transaction(() => {
        ...
        this.db
          .prepare(
            `INSERT INTO secrets (
               id, tenant_id, vault_id, type_id, internal_name, title,
  ...
  list(input: {
    ...
    if (input.vaultId !== undefined) {
      if (input.vaultId === null) where.push("vault_id IS NULL");
      else {
        where.push("vault_id = ?");
