...
import { startUpstreamServer, startVaultServer, login } from "./helpers.ts";
...
test("vault ui serves shared brand assets and fonts", async () => {
  const vault = await startVaultServer("vault-brand");
  try {
    const sharedPublicDir = path.resolve(process.cwd(), "..", "public");

    const indexResponse = await fetch(`${vault.baseUrl}/`);
    assert.equal(indexResponse.status, 200);
    const indexHtml = await indexResponse.text();
    assert.match(indexHtml, /href="\/brand\/favicon\.ico"/);

    const logoResponse = await fetch(`${vault.baseUrl}/brand/logo.png`);
    ...
    const faviconResponse = await fetch(`${vault.baseUrl}/brand/favicon.ico`);
    ...
    const fontResponse = await fetch(`${vault.baseUrl}/brand/fonts/source-sans-3/source-sans-3-v18-cyrillic_latin_latin-ext-regular.woff2`);
...
test("vault resolves the built UI path for the packaged server entrypoint", () => {
...
test("vault stores encrypted versions and never returns plaintext through metadata endpoints", async () => {
  const vault = await startVaultServer("vault-metadata");
  try {
    const session = await login(vault.baseUrl);
    ...
    const rotateAllowed = await fetch(`${vault.baseUrl}/v1/tenants/demo-tenant/secrets/deploy_token/versions`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${session.accessToken}`,
        "Content-Type": "application/json",
    ...
    const describe = await fetch(`${vault.baseUrl}/v1/tenants/demo-tenant/secrets/deploy_token`, {
...
test("vault exposes typed secret catalog, capabilities, and typed actions", async () => {
  const vault = await startVaultServer("vault-typed");
  const upstream = await startUpstreamServer();
  try {
    const session = await login(vault.baseUrl);
    const upstreamHost = new URL(upstream.baseUrl).host;

    const typesResponse = await fetch(`${vault.baseUrl}/v1/secret-types`);
    ...
    await createPolicy(vault.baseUrl, session.accessToken, {
    ...
    const capabilities = await fetch(`${vault.baseUrl}/v1/tenants/demo-tenant/secrets/revenuecat_primary/capabilities`, {
      headers: { Authorization: `Bearer ${session.accessToken}` },
    });
    ...
    const actions = await fetch(`${vault.baseUrl}/v1/tenants/demo-tenant/secrets/revenuecat_primary/actions`, {
      headers: { Authorization: `Bearer ${session.accessToken}` },
    });
    ...
    const run = await fetch(`${vault.baseUrl}/v1/tenants/demo-tenant/secrets/revenuecat_primary/actions/revenuecat.projects.list`, {
      method: "POST",
      headers: { Authorization: `Bearer ${session.accessToken}` },
    });
    ...
    assert.equal(typedResponse.platform, "clawjs-vault");
  } finally {
...
test("vault broker enforces deny precedence and host constraints", async () => {
  const vault = await startVaultServer("vault-broker");
  const upstream = await startUpstreamServer();
  try {
    const session = await login(vault.baseUrl);
    const upstreamHost = new URL(upstream.baseUrl).host;
    ...
    const ok = await fetch(`${vault.baseUrl}/v1/tenants/demo-tenant/broker/http`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${session.accessToken}`,
        "Content-Type": "application/json",
    ...
    const denied = await fetch(`${vault.baseUrl}/v1/tenants/demo-tenant/broker/http`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${session.accessToken}`,
        "Content-Type": "application/json",
    ...
    const hostMismatch = await fetch(`${vault.baseUrl}/v1/tenants/demo-tenant/broker/http`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${session.accessToken}`,
        "Content-Type": "application/json",
...
test("vault sidecar stays compatible with request/list/describe and supports process and browser leases", async () => {
  const vault = await startVaultServer("vault-sidecar");
  const upstream = await startUpstreamServer();
  try {
    const session = await login(vault.baseUrl);
    const upstreamHost = new URL(upstream.baseUrl).host;
    ...
    const principal = await createPrincipal(vault.baseUrl, session.accessToken, {
    ...
    const env = {
      ...
      VAULT_TOKEN: principal.principal.token,
    ...
    const scriptPath = path.join(fs.mkdtempSync(path.join(os.tmpdir(), "vault-sidecar-script-")), "stdin-reader.mjs");
    ...
    const screenshotPath = path.join(process.cwd(), "artifacts", "vault-browser-lease.png");
