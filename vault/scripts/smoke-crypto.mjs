...
console.log("Vault setup / unlock / recover / change-password");
const setup = vaultSetup("correct horse battery staple", {
  schemaVersion: 1,
  appVersion: "0.1.2",
  kdfParams: ARGON2_FAST_PARAMS,
  recoveryParams: ARGON2_FAST_PARAMS,
});
if (!setup.recoveryPhrase || setup.recoveryPhrase.split(" ").length !== 12) ko("12-word phrase"); else ok("12-word phrase");
...
try {
  const unlocked = vaultUnlock(setup.meta, "correct horse battery staple");
...
try {
  vaultUnlock(setup.meta, "wrong password");
  ko("unlock with wrong password (should throw)");
} catch (e) { ok("unlock with wrong password rejects"); }

try {
  const recovered = vaultRecover(setup.meta, setup.recoveryPhrase);
...
try {
  const changed = vaultChangePassword(setup.meta, "correct horse battery staple", "new-master-pw");
  const u = vaultUnlock(changed.newMeta, "new-master-pw");
