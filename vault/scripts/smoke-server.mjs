...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vault-server-"));
process.env.VAULT_DATA_DIR = tmpDir;
process.env.VAULT_DB_PATH = path.join(tmpDir, "vault.sqlite");
process.env.VAULT_PORT = "0"; // ephemeral
process.env.VAULT_HOST = "127.0.0.1";
...
if (h.ok && h.body.service === "clawjs-vault") ok("health"); else ko("health", h.body);

// Vault state pre-setup.
const s0 = await fetchJson(`${base}/v1/vault/state`);
if (s0.ok && !s0.body.initialized && !s0.body.unlocked) ok("vault state: uninitialized"); else ko("state", s0.body);

// Vault setup.
const setup = await fetchJson(`${base}/v1/vault/setup`, {
  method: "POST",
  body: JSON.stringify({ password: "master-pw" }),
});
if (setup.ok && setup.body.recoveryPhrase && setup.body.recoveryPhrase.split(" ").length === 12) ok("setup");
else ko("setup", setup.body);
const recoveryPhrase = setup.body.recoveryPhrase;

// State after setup.
const s1 = await fetchJson(`${base}/v1/vault/state`);
if (s1.body.initialized && s1.body.unlocked) ok("setup leaves vault unlocked"); else ko("post-setup", s1.body);
...
const cont = await fetchJson(`${base}/v1/tenants/clawix-local/vaults`, {
  method: "POST",
...
if (cont.status === 401) ok("auth required for vault create"); else ko("expected 401", cont.status);

// Lock.
const lockRes = await fetchJson(`${base}/v1/vault/lock`, { method: "POST" });
if (lockRes.ok) ok("lock"); else ko("lock", { status: lockRes.status, body: lockRes.body });

// Re-unlock with wrong password.
const u1 = await fetchJson(`${base}/v1/vault/unlock`, { method: "POST", body: JSON.stringify({ password: "wrong" }) });
if (u1.status === 401) ok("unlock rejects wrong password"); else ko("expected 401");

// Re-unlock with correct password.
const u2 = await fetchJson(`${base}/v1/vault/unlock`, { method: "POST", body: JSON.stringify({ password: "master-pw" }) });
if (u2.ok) ok("unlock with correct password"); else ko("unlock");

// Recover via phrase.
const lock2 = await fetchJson(`${base}/v1/vault/lock`, { method: "POST" });
if (lock2.ok) ok("lock #2");
const rec = await fetchJson(`${base}/v1/vault/recover`, { method: "POST", body: JSON.stringify({ phrase: recoveryPhrase }) });
if (rec.ok) ok("recover via phrase"); else ko("recover", rec.body);

// Change password.
const cp = await fetchJson(`${base}/v1/vault/change-password`, {
  method: "POST",
...
// Doctor.
const doc = await fetchJson(`${base}/v1/vault/doctor`);
