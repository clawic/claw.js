...
const BASE_URL = globalThis.__VAULT_BASE_URL__ || window.location.origin;
...
function readStoredSession() {
    const raw = window.localStorage.getItem("vault-session");
    if (!raw)
        return null;
    try {
...
export function App() {
    ...
    if (!session) {
        return _jsx(LoginScreen, { onLogin: (next) => {
                window.localStorage.setItem("vault-session", JSON.stringify(next));
                setSession(next);
...
function LoginScreen(props) {
    ...
    const [email, setEmail] = useState("admin@vault.local");
    const [password, setPassword] = useState("vault-admin");
