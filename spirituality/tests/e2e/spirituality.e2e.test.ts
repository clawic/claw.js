...
import { buildSpiritualityApp, SpiritualityClient } from "@clawjs/spirituality";

test("spirituality CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "spirituality-e2e-"));
  const dbPath = path.join(tmpDir, "spirituality.sqlite");
  ...
  try {
    ...
    const client = new SpiritualityClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "spirituality");
