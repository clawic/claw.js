...
import { buildWildlifeApp, WildlifeClient } from "@clawjs/wildlife";

test("wildlife CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "wildlife-e2e-"));
  const dbPath = path.join(tmpDir, "wildlife.sqlite");
  ...
  try {
    ...
    const client = new WildlifeClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "wildlife");
