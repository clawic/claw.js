...
import { buildCognitionApp, CognitionClient } from "@clawjs/cognition";

test("cognition CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "cognition-e2e-"));
  const dbPath = path.join(tmpDir, "cognition.sqlite");
  ...
  try {
    ...
    const client = new CognitionClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "cognition");
