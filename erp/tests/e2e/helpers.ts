...
import { buildErpApp } from "../../src/server/app.ts";

export async function startErpServer(prefix = "erp-e2e") {
  ...
  const { app } = buildErpApp({
    config: {
      ...
      dbPath: path.join(rootDir, ".data", "erp.sqlite"),
  ...
  const port = typeof address === "object" && address ? address.port : 4530;
