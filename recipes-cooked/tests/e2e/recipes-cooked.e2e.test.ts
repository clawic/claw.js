...
import { buildRecipesCookedApp, RecipesCookedClient } from "@clawjs/recipes-cooked";

test("recipes-cooked CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "recipes-cooked-e2e-"));
  const dbPath = path.join(tmpDir, "recipes-cooked.sqlite");
  ...
  try {
    ...
    const client = new RecipesCookedClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "recipes-cooked");
