...
import { buildSubscriptionsApp, SubscriptionsClient } from "@clawjs/subscriptions";
...
test("subscriptions CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new SubscriptionsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "subscriptions");
