...
import { buildHealthApp, HealthClient } from "@clawjs/health";

test("health CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "health-e2e-"));
  ...
  try {
    ...
    const client = new HealthClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "health");
