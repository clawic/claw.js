...
import { buildRecurringProblemsApp, RecurringProblemsClient } from "@clawjs/recurring-problems";
...
test("recurring-problems CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new RecurringProblemsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "recurring-problems");
