...
import { buildMentorshipApp, MentorshipClient } from "@clawjs/mentorship";
...
test("mentorship CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new MentorshipClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "mentorship");
