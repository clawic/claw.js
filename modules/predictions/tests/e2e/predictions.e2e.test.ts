...
import { buildPredictionsApp, PredictionsClient } from "@clawjs/predictions";
...
test("predictions CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new PredictionsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "predictions");
