...
import { buildRestaurantsApp, RestaurantsClient } from "@clawjs/restaurants";
...
test("restaurants CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new RestaurantsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "restaurants");
