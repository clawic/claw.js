...
import { buildConflictsApp, ConflictsClient } from "@clawjs/conflicts";
...
test("conflicts CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new ConflictsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "conflicts");
