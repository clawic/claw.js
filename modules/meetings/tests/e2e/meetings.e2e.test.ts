...
import { buildMeetingsApp, MeetingsClient } from "@clawjs/meetings";
...
test("meetings CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new MeetingsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "meetings");
