...
import { buildPossessionsApp, PossessionsClient } from "@clawjs/possessions";
...
test("possessions CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new PossessionsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "possessions");
