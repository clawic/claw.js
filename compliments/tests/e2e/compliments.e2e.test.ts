...
import { buildComplimentsApp, ComplimentsClient } from "@clawjs/compliments";

test("compliments CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "compliments-e2e-"));
  const dbPath = path.join(tmpDir, "compliments.sqlite");
  ...
  try {
    ...
    const client = new ComplimentsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "compliments");
