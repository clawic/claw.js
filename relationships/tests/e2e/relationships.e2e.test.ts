...
import { buildRelationshipsApp, RelationshipsClient } from "@clawjs/relationships";

test("relationships CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "relationships-e2e-"));
  const dbPath = path.join(tmpDir, "relationships.sqlite");
  ...
  try {
    ...
    const client = new RelationshipsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "relationships");
