...
import { buildBodyMeasuresApp, BodyMeasuresClient } from "@clawjs/body-measures";
...
test("body-measures CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new BodyMeasuresClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "body-measures");
