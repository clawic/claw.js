...
import { buildMemorableMomentsApp, MemorableMomentsClient } from "@clawjs/memorable-moments";
...
test("memorable-moments CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new MemorableMomentsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "memorable-moments");
