...
import { buildEventsAttendedApp, EventsAttendedClient } from "@clawjs/events-attended";
...
test("events-attended CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new EventsAttendedClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "events-attended");
