...
import { buildBeliefsApp, BeliefsClient } from "@clawjs/beliefs";

test("beliefs CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "beliefs-e2e-"));
  const dbPath = path.join(tmpDir, "beliefs.sqlite");
  ...
  try {
    ...
    const client = new BeliefsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "beliefs");
