...
import { buildScreenTimeApp, ScreenTimeClient } from "@clawjs/screen-time";
...
test("screen-time CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new ScreenTimeClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "screen-time");
