...
import { buildWritingApp, WritingClient } from "@clawjs/writing";

test("writing CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "writing-e2e-"));
  const dbPath = path.join(tmpDir, "writing.sqlite");
  ...
  try {
    ...
    const client = new WritingClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "writing");
