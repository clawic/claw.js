...
import { buildPetsApp, PetsClient } from "@clawjs/pets";

test("pets CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "pets-e2e-"));
  const dbPath = path.join(tmpDir, "pets.sqlite");
  ...
  try {
    ...
    const client = new PetsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "pets");
