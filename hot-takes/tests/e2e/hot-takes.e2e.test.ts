...
import { buildHotTakesApp, HotTakesClient } from "@clawjs/hot-takes";
...
test("hot-takes CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new HotTakesClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "hot-takes");
