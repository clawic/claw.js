...
import { buildPromisesApp, PromisesClient } from "@clawjs/promises";

test("promises CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "promises-e2e-"));
  const dbPath = path.join(tmpDir, "promises.sqlite");
  ...
  try {
    ...
    const client = new PromisesClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "promises");
