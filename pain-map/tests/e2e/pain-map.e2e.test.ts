...
import { buildPainMapApp, PainMapClient } from "@clawjs/pain-map";

test("pain-map CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "pain-map-e2e-"));
  const dbPath = path.join(tmpDir, "pain-map.sqlite");
  ...
  try {
    ...
    const client = new PainMapClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "pain-map");
