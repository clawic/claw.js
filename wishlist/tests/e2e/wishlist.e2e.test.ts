...
import { buildWishlistApp, WishlistClient } from "@clawjs/wishlist";

test("wishlist CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "wishlist-e2e-"));
  const dbPath = path.join(tmpDir, "wishlist.sqlite");
  ...
  try {
    ...
    const client = new WishlistClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "wishlist");
