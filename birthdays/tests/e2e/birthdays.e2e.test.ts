...
import { buildBirthdaysApp, BirthdaysClient } from "@clawjs/birthdays";

test("birthdays CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "birthdays-e2e-"));
  const dbPath = path.join(tmpDir, "birthdays.sqlite");
  ...
  try {
    ...
    const client = new BirthdaysClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "birthdays");
