...
import { buildMusicApp, MusicClient } from "@clawjs/music";

test("music CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "music-e2e-"));
  const dbPath = path.join(tmpDir, "music.sqlite");
  ...
  try {
    ...
    const client = new MusicClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "music");
