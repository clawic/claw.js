...
import { buildReadingMediaApp, ReadingMediaClient } from "@clawjs/reading-media";
...
test("reading-media CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new ReadingMediaClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "reading-media");
