...
import { buildPlantsApp, PlantsClient } from "@clawjs/plants";

test("plants CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "plants-e2e-"));
  const dbPath = path.join(tmpDir, "plants.sqlite");
  ...
  try {
    ...
    const client = new PlantsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "plants");
