...
function main() {
  ...
  for (const [runtimeId, runtime] of runtimes) {
    ...
    if ((runtime.id === "codex" || runtime.id === "hermes") && (runtime.recommended || runtime.production)) {
      errors.push(`${runtime.id} must keep recommended/production false until explicit evidence and product policy promote it`);
    }
  ...
  if (!support.includes("operable non-default runtime lens")) {
    errors.push("support matrix must describe Hermes as an operable non-default runtime lens, not a generic partial projection");
  }
  for (const snippet of [
    ...
    "recommended/production ecosystem claims still require final promotion policy"
  ]) {
