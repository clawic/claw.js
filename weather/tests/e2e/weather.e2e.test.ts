...
import { buildWeatherApp, WeatherClient } from "@clawjs/weather";

test("weather CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "weather-e2e-"));
  const dbPath = path.join(tmpDir, "weather.sqlite");
  ...
  try {
    ...
    const client = new WeatherClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "weather");
