...
async function makeFakeTarball(version: string): Promise<string> {
  ...
  await chmod(join(entryDir, "bin/clawjs-bridged"), 0o755);
  await writeFile(
    join(entryDir, "package.json"),
    JSON.stringify({ name: "clawjs-bridged-runtime", version, private: true }),
    "utf8",
  );
  await writeFile(join(entryDir, "INSTALL.txt"), "fake install instructions\n", "utf8");
  const tarball = join(dir, `clawjs-bridged-fake-${version}.tar.gz`);
  await execFileAsync("tar", ["-czf", tarball, "-C", dir, `clawjs-bridged-${version}`]);
  return tarball;
}
...
async function makeIsolatedHome(): Promise<{
  ...
  const prefix = join(home, "clawjs-bridged");
...
test("install.sh --help prints usage", async () => {
  ...
  assert.equal(code, 0);
  assert.match(stdout, /Install clawjs-bridged/);
...
test("install.sh extracts a tarball and creates a working symlink", async () => {
  ...
  try {
    ...
    const wrapper = await stat(join(h.prefix, "bin/clawjs-bridged"));
    assert.ok(wrapper.isFile());
    const link = await stat(join(h.binDir, "clawjs-bridged"));
...
test("install.sh --launchd writes a plist on macOS", async () => {
  ...
  try {
    ...
    const plistPath = join(
      h.home,
      "Library/LaunchAgents/com.clawjs.bridged.plist",
    );
    const plist = await readFile(plistPath, "utf8");
    assert.match(plist, /com\.clawjs\.bridged/);
    assert.match(plist, /CLAWJS_BRIDGE_PORT/);
...
test("install.sh --uninstall removes prefix + symlink + launchd plist", async () => {
  ...
  try {
    ...
    await assert.rejects(stat(join(h.binDir, "clawjs-bridged")));
    await assert.rejects(
      stat(join(h.home, "Library/LaunchAgents/com.clawjs.bridged.plist")),
    );
  } finally {
    await h.cleanup();
  }
