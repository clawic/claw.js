...
import { buildSmsApp } from "../../src/server/app.ts";

const SECRET = "sms-e2e-secret";
...
async function spinUp() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "sms-e2e-"));
  const built = buildSmsApp({ config: { host: "127.0.0.1", port: 0, dataDir: tmpDir, dbPath: path.join(tmpDir, "clawjs.sqlite"), sharedSecret: SECRET } });
  const client = new ChannelApiClient({ channel: "sms", baseUrl: "http://sms.test", token: SECRET, fetchImpl: injectFetch(built.app) });
  return { client, tmpDir, close: async () => { await built.app.close(); fs.rmSync(tmpDir, { recursive: true, force: true }); } };
}

test("sms: account CRUD round-trip", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary", credentialsRef: "vault://sms/primary" });
    assert.equal(account.channel, "sms");
...
test("sms: routing assign + resolve + remove", async () => {
  const ctx = await spinUp();
  try {
...
test("sms: send via stub transport records outbound message", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary" });
    const msg = await ctx.client.sendMessage({ accountId: account.id, targetId: "target-1", content: "hola desde sms" });
...
test("sms: inbound webhook lands in messages table", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary" });
...
test("sms: send to unknown account returns 404", async () => {
  const ctx = await spinUp();
  try {
