...
import { buildNutritionApp, NutritionClient } from "@clawjs/nutrition";

test("nutrition CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nutrition-e2e-"));
  const dbPath = path.join(tmpDir, "nutrition.sqlite");
  ...
  try {
    ...
    const client = new NutritionClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "nutrition");
