...
import { buildPhotographyApp, PhotographyClient } from "@clawjs/photography";
...
test("photography CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new PhotographyClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "photography");
