...
import { buildLessonsApp, LessonsClient } from "@clawjs/lessons";

test("lessons CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "lessons-e2e-"));
  ...
  try {
    ...
    const client = new LessonsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "lessons");
