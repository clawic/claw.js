...
import { buildCommStatsApp, CommStatsClient } from "@clawjs/comm-stats";

test("comm-stats CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "comm-stats-e2e-"));
  const dbPath = path.join(tmpDir, "comm-stats.sqlite");
  ...
  try {
    ...
    const client = new CommStatsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "comm-stats");
