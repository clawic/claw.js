...
import { buildStretchingApp, StretchingClient } from "@clawjs/stretching";

test("stretching CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "stretching-e2e-"));
  const dbPath = path.join(tmpDir, "stretching.sqlite");
  ...
  try {
    ...
    const client = new StretchingClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "stretching");
