// SecretsResolver: high-level surface used by the HTTP layer and the
...
export class SecretsResolver {
  ...
  readonly containers: FolderStore;
  ...
  constructor(db: SqliteDb) {
    ...
    this.containers = new FolderStore(db);
  ...
  describeContainer(row: FolderRow) {
    return {
      id: row.id,
      tenantId: row.tenant_id,
      name: row.name,
