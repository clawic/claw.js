// SecretsSession holds the unlocked masterKey + auditMacKey for the running
// secrets server. Lock zeroes both. A simple inactivity timer can re-lock
...
export class SecretsSession {
  ...
  requireKeys(tenantId?: string): { masterKey: LockableSecret; auditMacKey: LockableSecret; tenantId: string } {
    if (this.state.kind !== "unlocked") throw new Error("Secrets is locked");
    if (tenantId && tenantId !== this.state.tenantId) {
      throw new Error(`Secrets is unlocked for tenant ${this.state.tenantId}, not ${tenantId}`);
    }
    if (!this.masterKey || !this.auditMacKey) throw new Error("Secrets session keys missing");
    this.touch();
