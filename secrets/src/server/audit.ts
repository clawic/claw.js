...
//     SecretsMetaSnapshot.auditChainGenesis.
...
export class AuditStore {
  ...
  private prevHash(tenantId: string, meta: SecretsMetaSnapshot, sequence: number): Uint8Array {
  ...
  append(input: AppendInput): DecryptedAuditEvent {
    ...
    const row: AuditEventRow = {
      ...
      folder_id: input.event.folderId ?? null,
    ...
    this.db
      .prepare(
        `INSERT INTO audit_events (
           id, tenant_id, secret_id, folder_id, version_id, kind, timestamp, source,
