...
import { TenantStore, SecretsMetaStore } from "../src/server/stores.ts";
...
const e1 = audit.append({
  ...
  event: { kind: "secretsSetup", source: "system", success: true, payload: { setup: true } },
});
...
if (events[0].kind === "uiCopy" && events[2].kind === "secretsSetup") ok("events sorted desc"); else ko("events sorted desc");
