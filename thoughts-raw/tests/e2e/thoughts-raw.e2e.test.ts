...
import { buildThoughtsRawApp, ThoughtsRawClient } from "@clawjs/thoughts-raw";

test("thoughts-raw CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "thoughts-raw-e2e-"));
  const dbPath = path.join(tmpDir, "thoughts-raw.sqlite");
  ...
  try {
    ...
    const client = new ThoughtsRawClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "thoughts-raw");
