...
if (!group || argv.includes("--help")) {
  process.stdout.write([
    "Usage: slack <command> [options]",
    "  slack serve [--port N] [--secret TOKEN]",
    "  slack accounts add --name N [--credentials-ref SECRET]",
    "  slack accounts list",
    "  slack accounts remove --id ID",
    "  slack routing assign --account ID --target T --agent A",
    "  slack routing list [--account ID] [--agent A]",
    "  slack send --account ID --target T --content TEXT",
    "  slack messages [--account ID] [--target T] [--direction inbound|outbound]",
  ].join("\n") + "\n");
  process.exit(0);
}
async function main() {
  if (group === "serve") {
    const { app } = buildSlackApp({ config: { ...(flags.port ? { port: Number(flags.port) } : {}), ...(flags.secret ? { sharedSecret: flags.secret } : {}) } });
    const addr = await app.listen({ host: flags.host ?? "127.0.0.1", port: flags.port ? Number(flags.port) : 4701 });
    process.stdout.write(`${addr}\n`); return;
  }
  const baseUrl = flags.url ?? "http://127.0.0.1:4701"; const token = flags.token ?? flags.secret ?? "";
  const client = new ChannelApiClient({ channel: "slack", baseUrl, token });
