...
if (!group || argv.includes("--help")) {
  process.stdout.write([
    ...
    "  matrix serve [--port N] [--secret TOKEN]",
    "  matrix accounts add --name N [--credentials-ref SECRET]",
    "  matrix accounts list",
    "  matrix accounts remove --id ID",
    "  matrix routing assign --account ID --target T --agent A",
    "  matrix routing list [--account ID] [--agent A]",
    "  matrix send --account ID --target T --content TEXT",
    "  matrix messages [--account ID] [--target T] [--direction inbound|outbound]",
  ].join("\n") + "\n");
  process.exit(0);
}
async function main() {
  if (group === "serve") {
    const { app } = buildMatrixApp({ config: { ...(flags.port ? { port: Number(flags.port) } : {}), ...(flags.secret ? { sharedSecret: flags.secret } : {}) } });
    const addr = await app.listen({ host: flags.host ?? "127.0.0.1", port: flags.port ? Number(flags.port) : 4708 });
    process.stdout.write(`${addr}\n`); return;
  }
  const baseUrl = flags.url ?? "http://127.0.0.1:4708"; const token = flags.token ?? flags.secret ?? "";
  const client = new ChannelApiClient({ channel: "matrix", baseUrl, token });
