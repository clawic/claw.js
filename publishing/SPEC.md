# clawjs-publishing · publication framework spec
...
- Auth: opaque bearer token issued at install time, stored at `~/.config/clawjs-publishing/token`.
- Secrets at rest: provider OAuth tokens go through `clawjs-vault`; Publishing stores vault key references, never plaintext.
...
Signature header: `X-Publishing-Signature: t=<unix>,v1=<hex>` over `t.body` with the per-webhook secret from vault.

## CLI surface

See `src/cli/commands/` and `publishing --help`. Every API capability has a CLI command. JSON-out by default (`--json`); pretty-print when stdout is a TTY.
