...
test("approvals: all-required stage waits for every required reviewer", async () => {
  const { baseUrl, adminToken, cleanup } = await bootTestApp();
  try {
    const ws = await jsonFetch<{ workspace: { id: string } }>(baseUrl, adminToken, "/v1/workspaces", {
      method: "POST",
      body: JSON.stringify({ name: "approval-all-required-ws" }),
    });
    const workspaceId = ws.body.workspace.id;
    const connect = await jsonFetch<{ account: { id: string } }>(baseUrl, adminToken, `/v1/workspaces/${workspaceId}/channels/connect/devnull`, {
      method: "POST",
    ...
    const wf = await jsonFetch<{ workflow: { id: string } }>(baseUrl, adminToken, `/v1/workspaces/${workspaceId}/approvals/workflows`, {
      method: "POST",
      body: JSON.stringify({
        name: "copy-review-pair",
        stages: [
          { order: 1, name: "copy", all_required: true, required_user_ids: ["u1", "u2"] },
        ],
      }),
    });
    const start = await jsonFetch<{ id: string }>(baseUrl, adminToken, `/v1/workspaces/${workspaceId}/approvals`, {
    ...
    const first = await jsonFetch<{ finalState: string }>(baseUrl, adminToken, `/v1/workspaces/${workspaceId}/approvals/${start.body.id}/decisions`, {
      method: "POST",
      body: JSON.stringify({ stage_index: 0, reviewer_user_id: "u1", decision: "approve" }),
    });
    assert.equal(first.body.finalState, "pending");

    const second = await jsonFetch<{ finalState: string }>(baseUrl, adminToken, `/v1/workspaces/${workspaceId}/approvals/${start.body.id}/decisions`, {
      method: "POST",
      body: JSON.stringify({ stage_index: 0, reviewer_user_id: "u2", decision: "approve" }),
    });
    assert.equal(second.body.finalState, "approved");
  } finally {
    await cleanup();
  }
});
