...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/members", async (req, reply) => {
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/members", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { members: s().members.list(wsParam(req)) };
  });
  app.delete<{ Params: { ws: string; userId: string } }>("/v1/workspaces/:ws/members/:userId", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/invitations", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    const parsed = memberInput.safeParse(req.body);
    if (!parsed.success) return badRequest(reply, parsed.error.issues);
    return reply.code(201).send({ invitation: s().invitations.create(wsParam(req), parsed.data.email, parsed.data.role) });
  });
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/invitations", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { invitations: s().invitations.list(wsParam(req)) };
  });

  // ───── Tokens ─────────────────────────────────────────────────────────────
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/tokens", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/tokens", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { tokens: s().auth.listTokens(wsParam(req)) };
  });
  app.delete<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/tokens/:id", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  // ───── Audit ──────────────────────────────────────────────────────────────
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/audit", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { events: s().audit.list(wsParam(req)) };
  });

  // ───── Channels ───────────────────────────────────────────────────────────
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/channels", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.post<{ Params: { ws: string; family: string } }>("/v1/workspaces/:ws/channels/connect/:family", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/channels", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { accounts: s().channels.list(wsParam(req)) };
  });
  app.get<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/channels/:id", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    const account = s().channels.get(wsParam(req), req.params.id);
    if (!account) return reply.code(404).send({ error: "not_found" });
    return account;
  });
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/channels/:id/probe", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    s().jobs.enqueue({
      workspaceId: wsParam(req),
      kind: "health_probe",
  ...
  app.delete<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/channels/:id", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  // ───── Posts ──────────────────────────────────────────────────────────────
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/posts", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/posts", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/posts/:id", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.patch<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/posts/:id", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return s().posts.update(req.params.ws, req.params.id, (req.body ?? {}) as Record<string, never>);
  });
  app.delete<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/posts/:id", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/posts/:id/schedule", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/posts/:id/unschedule", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { post: s().posts.unschedule(req.params.ws, req.params.id) };
  });
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/posts/:id/cancel", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { post: s().posts.cancel(req.params.ws, req.params.id) };
  });
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/posts/:id/duplicate", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/posts/:id/retry", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/posts/:id/editorial", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.put<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/posts/:id/variants", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  // ───── Media ──────────────────────────────────────────────────────────────
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/media", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/media", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { media: s().media.list(wsParam(req), (req.query as { mime?: string }).mime) };
  });
  app.get<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/media/:id", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  // ───── Queues + blackouts ─────────────────────────────────────────────────
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/queues", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    const parsed = queueInput.safeParse(req.body);
    if (!parsed.success) return badRequest(reply, parsed.error.issues);
    return reply.code(201).send({ queue: s().queues.create(wsParam(req), parsed.data) });
  });
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/queues", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { queues: s().queues.list(wsParam(req)) };
  });
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/queues/:id/slots", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/queues/:id/slots", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { slots: s().queues.listSlots(req.params.id) };
  });
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/queues/:id/accounts", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    const body = req.body as { channel_account_id: string };
    s().queues.attachAccount(req.params.id, body.channel_account_id);
    return { ok: true };
  });
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/queues/:id/entries", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/queues/:id/pause", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    s().queues.pause(req.params.id);
    s().events.emit({ workspaceId: wsParam(req), name: "queue.paused", data: { queue_id: req.params.id } });
    return { ok: true };
  });
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/queues/:id/resume", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    s().queues.resume(req.params.id);
    s().events.emit({ workspaceId: wsParam(req), name: "queue.resumed", data: { queue_id: req.params.id } });
    return { ok: true };
  });
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/blackouts", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/labels", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    const parsed = labelInput.safeParse(req.body);
    if (!parsed.success) return badRequest(reply, parsed.error.issues);
    return reply.code(201).send({ label: s().labels.create(wsParam(req), parsed.data) });
  });
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/labels", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { labels: s().labels.list(wsParam(req)) };
  });
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/campaigns", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/campaigns", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { campaigns: s().campaigns.list(wsParam(req)) };
  });
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/campaigns/:id/posts", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    const body = req.body as { post_id: string };
    s().campaigns.addPost(req.params.id, body.post_id);
    return { ok: true };
  });
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/templates", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/templates", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { templates: s().templates.list(wsParam(req)) };
  });
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/templates/:id/apply", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/hashtag-groups", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/hashtag-groups", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { groups: s().hashtagGroups.list(wsParam(req)) };
  });
  app.put<{ Params: { ws: string } }>("/v1/workspaces/:ws/dynamic-variables", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/dynamic-variables", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { variables: s().dynamicVariables.list(wsParam(req)) };
  });
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/utm-templates", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/utm-templates", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { templates: s().utm.list(wsParam(req)) };
  });

  // ───── Recurrences, evergreen, A/B ────────────────────────────────────────
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/recurrences", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/recurrences", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { recurrences: s().recurrences.list(wsParam(req)) };
  });
  app.delete<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/recurrences/:id", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/evergreen-pools", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/evergreen-pools/:id/posts", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    const body = req.body as { post_id: string };
    s().evergreen.addMember(req.params.id, body.post_id);
    return { ok: true };
  });
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/ab-sets", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    const body = req.body as Parameters<AbTestsService["create"]>[1];
    return reply.code(201).send({ set: s().ab.create(wsParam(req), body) });
  });
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/ab-sets/:id/arms", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  // ───── Imports ────────────────────────────────────────────────────────────
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/imports/json", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  // ───── Inbox ──────────────────────────────────────────────────────────────
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/inbox/threads", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/inbox/threads/:id/messages", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { messages: s().inbox.listMessages(req.params.id) };
  });
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/inbox/threads/:id/messages", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  // ───── Approvals ──────────────────────────────────────────────────────────
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/approvals/workflows", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/approvals", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/approvals/:id/decisions", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  // ───── Webhooks ───────────────────────────────────────────────────────────
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/webhooks", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/webhooks", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { webhooks: s().webhooks.list(wsParam(req)) };
  });
  app.get<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/webhooks/:id/deliveries", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { deliveries: s().webhooks.deliveries(req.params.id) };
  });
  app.post<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/webhooks/:id/replay", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/metrics/accounts", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string; id: string } }>("/v1/workspaces/:ws/metrics/posts/:id", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { rows: s().analytics.postMetrics(req.params.id) };
  });
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/suggest-time", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    const q = req.query as { account: string };
    return { suggestions: s().analytics.suggestBestTimes(q.account) };
  });
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/reports", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/reports", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { reports: s().reports.list(wsParam(req)) };
  });

  // ───── Integrations + brand voices ────────────────────────────────────────
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/integrations", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/integrations", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { integrations: s().integrations.list(wsParam(req)) };
  });
  app.post<{ Params: { ws: string } }>("/v1/workspaces/:ws/brand-voices", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/brand-voices", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { voices: s().brandVoices.list(wsParam(req)) };
  });

  // ───── Jobs (read-only inspector) ─────────────────────────────────────────
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/jobs", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
  ...
  app.get<{ Params: { ws: string } }>("/v1/workspaces/:ws/settings", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
    return { settings: s().settings.list("workspace", wsParam(req)) };
  });
  app.put<{ Params: { ws: string } }>("/v1/workspaces/:ws/settings", async (req, reply) => {
    if (!authorizedFor(req.principal, wsParam(req))) return reply.code(403).send({ error: "forbidden" });
