...
export async function webhooks(ctx: CliContext): Promise<number> {
  ...
  if (!command || command === "list") {
    out(ctx, await ctx.client.get(`/v1/ws/${ws}/webhooks`));
    return 0;
  }
  if (command === "create") {
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/webhooks`, {
  ...
  if (command === "deliveries") {
    ...
    out(ctx, await ctx.client.get(`/v1/ws/${ws}/webhooks/${id}/deliveries`));
    return 0;
  }
  if (command === "replay") {
    if (!id) { ctx.host.stderr.write("webhook id required\n"); return 64; }
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/webhooks/${id}/replay`, { delivery_id: ctx.args.flags.delivery }));
    return 0;
  }
