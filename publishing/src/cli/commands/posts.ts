...
export async function posts(ctx: CliContext): Promise<number> {
  ...
  if (!command || command === "list") {
    ...
    out(ctx, await ctx.client.get(`/v1/ws/${ws}/posts${params.length ? `?${params.join("&")}` : ""}`));
    return 0;
  }
  if (command === "create") {
    const file = readFileFlag(ctx, "file") as Record<string, unknown> | null;
    if (file) {
      out(ctx, await ctx.client.post(`/v1/ws/${ws}/posts`, file));
      return 0;
    }
    ...
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/posts`, {
      accounts: [account],
  ...
  if (command === "get") {
    ...
    out(ctx, await ctx.client.get(`/v1/ws/${ws}/posts/${id}`));
    return 0;
  }
  if (command === "update") {
    const id = idOrSub;
    ...
    out(ctx, await ctx.client.patch(`/v1/ws/${ws}/posts/${id}`, file ?? {}));
    return 0;
  }
  if (command === "delete") {
    const id = idOrSub;
    out(ctx, await ctx.client.delete(`/v1/ws/${ws}/posts/${id}`));
    return 0;
  }
  if (command === "duplicate") {
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/posts/${idOrSub}/duplicate`));
    return 0;
  }
  if (command === "schedule") {
    const at = ctx.args.flags.at;
    const tz = ctx.args.flags.tz;
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/posts/${idOrSub}/schedule`, { at, tz }));
    return 0;
  }
  if (command === "schedule-now") {
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/posts/${idOrSub}/schedule`, { now: true }));
    return 0;
  }
  if (command === "unschedule") {
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/posts/${idOrSub}/unschedule`));
    return 0;
  }
  if (command === "cancel") {
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/posts/${idOrSub}/cancel`));
    return 0;
  }
  if (command === "retry") {
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/posts/${idOrSub}/retry`, { account: ctx.args.flags.account }));
    return 0;
  }
  if (command === "approve") {
    const stage = ctx.args.flags.stage;
    const decision = ctx.args.flags.decision ?? "approve";
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/approvals/${idOrSub}/decisions`, { stage_index: Number(stage ?? "0"), reviewer_user_id: ctx.args.flags.user ?? "cli", decision, comment: ctx.args.flags.comment }));
    return 0;
  }
  if (command === "variants" && idOrSub === "set") {
    ...
    out(ctx, await ctx.client.put(`/v1/ws/${ws}/posts/${postId}/variants`, { channel_account_id: ctx.args.flags.account ?? null, variant: file }));
    return 0;
  }
