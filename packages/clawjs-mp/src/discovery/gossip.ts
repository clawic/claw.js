// LAN gossip for mp/2.0.0.
//
// Discovery on the same network segment goes via mDNS (Bonjour). Each peer
// advertises a `_clawix-mp._udp.local` service with its handle fingerprint and
...
export const CLAWIX_MP_SERVICE_TYPE = "clawix-mp" as const;
...
export class MdnsGossip implements GossipClient {
  ...
  async start(): Promise<void> {
    ...
    if (this.opts.browse !== false) {
      this.browser = this.bonjour!.find({ type: CLAWIX_MP_SERVICE_TYPE }, (svc) => {
