...
test("runtime global data fallback uses the central global data storage helper", () => {
...
test("runtime config preserves service and shared global data override precedence", () => {
  withPatchedEnv(
    {
      RUNTIME_DATA_DIR: path.join(os.tmpdir(), "runtime-service-data"),
      CLAW_DATA_DIR: path.join(os.tmpdir(), "runtime-shared-data"),
      CLAW_HOME: path.join(os.tmpdir(), "runtime-home"),
    },
    () => {
      assert.equal(loadRuntimeConfig().dataDir, path.join(os.tmpdir(), "runtime-service-data"));
    },
  );
  ...
  withPatchedEnv(
    ...
    () => {
      assert.equal(loadRuntimeConfig().dataDir, path.join(os.tmpdir(), "runtime-shared-data"));
    },
  );
  ...
  withPatchedEnv(
    ...
    () => {
      assert.equal(loadRuntimeConfig().dataDir, path.join(os.tmpdir(), "runtime-home", "data"));
    },
  );
});
