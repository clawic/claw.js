// In-process broker for marketplace/1.0.0 Phase 1 tests, plus a thin client for the
...
// `POST /marketplace/publish` and serves an array of matching intents on
// `POST /marketplace/query` keyed by `discovery_key`.
//
...
export class RemoteBrokerClient {
  ...
  async publish(intent: Intent): Promise<void> {
    ...
    const res = await this.fetcher(`${this.opts.baseUrl}/marketplace/publish`, {
      method: "POST",
  ...
  async query(opts: { discoveryKey: Uint8Array; limit?: number }): Promise<Intent[]> {
    ...
    const res = await this.fetcher(`${this.opts.baseUrl}/marketplace/query`, {
      method: "POST",
      headers: { "Content-Type": "application/cbor", ...(this.opts.headers ?? {}) },
      body,
      signal: this.signal(),
  ...
  async health(): Promise<RemoteBrokerHealth> {
    try {
      const res = await this.fetcher(`${this.opts.baseUrl}/marketplace/health`, { signal: this.signal() });
...
/**
 * Decode the payload of a `POST /marketplace/query` body sent by `RemoteBrokerClient`.
