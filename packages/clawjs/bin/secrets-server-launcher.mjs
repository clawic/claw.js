...
function defaultClawjsDataRoot(flags) {
  const explicit = flags["data-dir"] ?? process.env.SECRETS_DATA_DIR ?? process.env.CLAW_DATA_DIR ?? process.env.CLAWJS_MAIN_DATA_DIR ?? process.env.CLAWIX_CLAW_DATA_DIR ?? process.env.CLAWIX_CLAWJS_DATA_DIR;
