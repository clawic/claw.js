...
import { runRuntimePortalCli } from "./cli-runtime-portal-command.ts";
...
async function runCliUnsafe(argv: string[], context: CliContext): Promise<number> {
  ...
  if (runtimePortalExit !== null) return runtimePortalExit;
  ...
  if (group === "workspace" && command === "discover") {
    ...
    const maxDepthFlag = flags["max-depth"];
    const maxDepth = maxDepthFlag !== undefined ? Number(maxDepthFlag) : undefined;
    if (maxDepthFlag !== undefined && (!/^\d+$/.test(maxDepthFlag) || !Number.isSafeInteger(maxDepth))) {
