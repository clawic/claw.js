...
function surfaceNodeSearchDocument(node: ClawPersistentSurfaceNode): SearchDocumentInput {
  ...
  return {
    ...
    body: [
      ...
      locator,
      node.notes,
      node.resourceContract?.startup,
      node.resourceContract?.idle,
      node.resourceContract?.memory,
      node.resourceContract?.streaming,
      node.resourceContract?.storage,
      node.resourceContract?.hotPath,
      node.resourceContract?.scale,
      node.resourceContract?.validation,
...
function surfaceRouteSearchDocument(route: ClawSurfaceRoute): SearchDocumentInput {
  ...
  return {
    ...
    body: [
      ...
      ...adrs,
      ...gaps,
      route.resourceContract?.startup,
      route.resourceContract?.idle,
      route.resourceContract?.memory,
      route.resourceContract?.streaming,
      route.resourceContract?.storage,
      route.resourceContract?.hotPath,
      route.resourceContract?.scale,
      route.resourceContract?.validation,
      route.notes,
    ].filter(Boolean).join("\n"),
    resourceId: route.id,
    path: sourcePath,
    metadata: {
      routeSteward: route.steward,
