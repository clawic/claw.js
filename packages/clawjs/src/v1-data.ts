...
function shouldHandleV1DataCommand(group: string | undefined, command: string | undefined, wantsHelp: boolean): group is string {
  ...
  const commandsByGroup: Record<string, Set<string>> = {
    ...
    agents: new Set(["list", "get", "upsert", "delete", "schema", "evaluate-access", "delegation-check", "route-check", "resolve-external-identity", "project-support-inbox", "memory-check", "budget-check", "surface-projection", "config-revision", "incident", "activity-feed", "blueprint", "evaluation", "retirement-plan", "help"]),
