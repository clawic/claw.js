...
test("runCli exposes targeted runtime portals for OpenClaw, Codex, and Hermes", async (t) => {
  ...
  for (const action of ["preview", "resolve", "history"]) {
    const stdout = captureStream();
    assert.equal(await runCli(["runtime", "hermes", "sessions", action, "--session-key", "missing-session", "--workspace", workspaceRoot, "--home-dir", missingHermesHome, "--json"], {
      stdout: stdout.stream,
      stderr: captureStream().stream,
      cwd: process.cwd(),
    }), CLI_EXIT_DEGRADED);
    const payload = JSON.parse(stdout.getOutput()) as {
      data: {
        ...
        degradedReason?: string;
        writesRuntime?: boolean;
        result?: { found?: boolean; totalProjected?: number };
        safeDefault?: string;
        claimEffect?: string;
      };
    };
    assert.equal(payload.data.status, "degraded", action);
    assert.equal(payload.data.degradedReason, "runtime_cli_unavailable_or_session_store_missing", action);
    assert.equal(payload.data.writesRuntime, false, action);
    assert.equal(payload.data.result?.found, false, action);
    if (action === "history") assert.equal(payload.data.result?.totalProjected, 0, action);
  ...
  assert.equal(commandsPayload.data.executableByClawCli.find((entry) => entry.command === "runtime codex sessions create --title <title> --confirm-runtime-write")?.writesRuntime, false);
  assert.equal(commandsPayload.data.executableByClawCli.find((entry) => entry.command === "runtime codex sessions create --title <title> --confirm-runtime-write")?.wouldWriteRuntime, true);
    ...
    data: {
      ...
      domainData: {
        ...
        sessions?: {
          ...
          sessions?: Array<{ id: string; kind: string; path: string; enabled?: boolean; summary?: string; provenance?: { source?: string }; limitations?: string[] }>;
        };
        skills?: {
        ...
        gateway?: {
          resources?: Array<{ id: string; status?: string; kind?: string; path?: string; summary?: string; limitations?: string[]; attributes?: string[]; nativeIdentifier?: { name?: string }; provenance?: { path?: string }; transportPolicy?: { id?: string } }>;
        ...
        doctorCompat?: {
          resources?: Array<{ id: string; status?: string; kind?: string; path?: string; summary?: string; limitations?: string[]; attributes?: string[]; nativeIdentifier?: { name?: string }; provenance?: { path?: string } }>;
        };
        sandboxPermissions?: {
          permissionMode?: string;
          resources?: Array<{ id: string; status?: string; kind?: string; path?: string; summary?: string; attributes?: string[]; nativeIdentifier?: { name?: string }; provenance?: { path?: string } }>;
        };
        configuration?: {
  ...
  for (const externalDomain of ["channels", "providers", "auth", "models"]) {
    ...
    assert.equal(hermesPayload.data.support?.ecosystem?.evidenceRequirements?.find((entry) => entry.id === `hermes.${externalDomain}.live_evidence`)?.approvalRequired, true);
  }
  ...
  for (const externalDomain of ["channels", "providers", "auth", "models"]) {
    assert.equal(hermesPayload.data.supportAudit?.blockerSummary?.externalPendingDomains?.includes(externalDomain), true);
  }
  ...
  assert.equal(hermesPayload.data.supportAudit?.finalPromotionReview?.externalPendingRequirementIds?.includes("hermes.models.live_evidence"), true);
  ...
  assert.equal(hermesReentryPacket("hermes.sessions.create.action_contract")?.officialTransportSurface, "stdio_or_websocket_json_rpc");
  assert.deepEqual(hermesReentryPacket("hermes.sessions.create.action_contract")?.officialTransportClasses, ["stdio_json_rpc", "websocket_json_rpc"]);
  assert.equal(hermesReentryPacket("hermes.sessions.create.action_contract")?.officialTransportSource, "https://hermes-agent.nousresearch.com/docs/developer-guide/programmatic-integration");
  ...
  assert.equal(hermesReentryPacket("hermes.sessions.create.action_contract")?.productionTransportBlocker, "approval_required_for_non_loopback_endpoint_and_lifecycle_management");
  ...
  assert.equal(hermesReentryPacket("hermes.sessions.send.action_contract")?.officialTransportSurface, "stdio_or_websocket_json_rpc");
  assert.equal(hermesReentryPacket("hermes.sessions.send.action_contract")?.productionTransportBlocker, "approval_required_for_non_loopback_endpoint_and_lifecycle_management");
  ...
  assert.equal(hermesPayload.data.domains.find((entry) => entry.domain === "gateway")?.count, 2);
  ...
  assert.equal(hermesPayload.data.domainData.sessions?.sessions?.[0]?.path?.endsWith("runtime-session.jsonl"), true);
  assert.equal(hermesPayload.data.domainData.sessions?.sessions?.[0]?.enabled, true);
  assert.equal(hermesPayload.data.domainData.sessions?.sessions?.[0]?.summary?.includes("content is not included by default"), true);
  ...
  assert.equal(hermesSkillResource?.provenance?.source, "hermes-runtime-adapter");
  assert.equal(hermesSkillResource?.limitations?.includes("validation: snapshot_required"), true);
  const hermesMemoryResource = hermesPayload.data.domainData.memory?.memory?.find((entry) => entry.id === "hermes-memory-profile");
  assert.equal(hermesMemoryResource?.summary?.includes("not exposed by default"), true);
  assert.equal(hermesMemoryResource?.nativeIdentifier?.name, "memoryId");
  assert.equal(hermesMemoryResource?.provenance?.source, "hermes-runtime-adapter");
  assert.equal(hermesPayload.data.status.capabilityMap?.memory?.status, "degraded");
  assert.equal(hermesPayload.data.domains.find((entry) => entry.domain === "memory")?.status, "degraded");
  const hermesProviderResource = hermesPayload.data.domainData.providers?.providers?.find((entry) => entry.id === "openai");
  ...
  assert.equal(hermesDefaultModelResource?.nativeIdentifier?.name, "modelId");
  assert.equal(hermesDefaultModelResource?.provenance?.source, "hermes-runtime-adapter");
  const hermesFallbackModelResource = hermesPayload.data.domainData.models?.models?.find((entry) => entry.id === "anthropic/claude-3-5-sonnet");
  assert.equal(hermesFallbackModelResource?.source, "config");
  assert.equal(hermesFallbackModelResource?.provenance?.source, "hermes-runtime-adapter");
  const hermesSchedulerResource = hermesPayload.data.domainData.scheduler?.schedulers?.find((entry) => entry.id === "daily-summary");
  assert.equal(hermesSchedulerResource?.kind, "cron");
  assert.equal(hermesSchedulerResource?.nativeIdentifier?.name, "schedulerId");
  assert.equal(hermesSchedulerResource?.provenance?.source, "hermes-runtime-adapter");
  const hermesPluginResource = hermesPayload.data.domainData.plugins?.plugins?.find((entry) => entry.id === "mcp-github");
  assert.equal(hermesPluginResource?.nativeIdentifier?.name, "pluginId");
  assert.equal(hermesPluginResource?.provenance?.source, "hermes-runtime-adapter");
  const hermesChannelResource = hermesPayload.data.domainData.channels?.channels?.find((entry) => entry.id === "slack");
  ...
  assert.equal(hermesPayload.data.domainData.gateway?.resources?.find((entry) => entry.id === "gateway-status")?.path?.endsWith("config.yaml"), true);
  assert.equal(hermesPayload.data.domainData.gateway?.resources?.find((entry) => entry.id === "gateway-status")?.provenance?.path?.endsWith("config.yaml"), true);
  ...
  assert.equal(hermesPayload.data.domainData.gateway?.resources?.find((entry) => entry.id === "tui-gateway-transport-policy")?.path?.endsWith("config.yaml"), true);
  assert.equal(hermesPayload.data.domainData.gateway?.resources?.find((entry) => entry.id === "tui-gateway-transport-policy")?.provenance?.path?.endsWith("config.yaml"), true);
  ...
  assert.equal(hermesPayload.data.domainData.sandboxPermissions?.resources?.find((entry) => entry.id === "sandbox-policy")?.path, workspaceRoot);
  assert.equal(hermesPayload.data.domainData.sandboxPermissions?.resources?.find((entry) => entry.id === "sandbox-policy")?.provenance?.path, workspaceRoot);
    ...
    const resourcePayload = JSON.parse(resourceStdout.getOutput()) as {
      data: {
        ...
        data: {
          ...
          memory?: unknown[];
          schedulers?: unknown[];
          resources?: Array<{ id?: string; attributes?: string[]; transportPolicy?: { id?: string } }>;
  ...
  for (const externalDomain of ["channels", "providers", "auth", "models"]) {
    assert.equal(hermesSupportPayload.data.blockerSummary.externalPendingDomains?.includes(externalDomain), true);
  }
  ...
  assert.equal(hermesSupportPayload.data.evidenceRequirements?.some((entry) => entry.id === "hermes.providers.live_evidence" && entry.approvalRequired === true), true);
  assert.equal(hermesSupportPayload.data.evidenceRequirements?.some((entry) => entry.id === "hermes.auth.live_evidence" && entry.approvalRequired === true), true);
  assert.equal(hermesSupportPayload.data.evidenceRequirements?.some((entry) => entry.id === "hermes.models.live_evidence" && entry.approvalRequired === true), true);
  ...
  assert.equal(hermesSupportPayload.data.domains?.find((entry) => entry.domain === "models")?.evidenceRequirementIds?.includes("hermes.models.live_evidence"), true);
  ...
  assert.equal(hermesSupportPayload.data.domains?.find((entry) => entry.domain === "memory")?.readProjectionStatus, "degraded_projection");
  ...
  assert.equal(hermesSupportPayload.data.projectionSummary?.byReadProjectionStatus?.projected, 2);
  assert.equal(hermesSupportPayload.data.projectionSummary?.byReadProjectionStatus?.degraded_projection, 11);
  ...
  assert.equal(hermesSupportPayload.data.projectionSummary?.implementedFacetCounts?.ready_runtime_projection, 1);
  assert.equal(hermesSupportPayload.data.projectionSummary?.implementedFacetCounts?.degraded_runtime_projection, 11);
  ...
  assert.equal(hermesSupportPayload.data.evidenceReadinessSummary?.upstreamContractRequirementIds?.includes("hermes.sessions.create.action_contract"), false);
  ...
  assert.equal(hermesSupportPayload.data.evidenceReadinessSummary?.tuiGatewayWrapperBlockedCount, 4);
  assert.equal(hermesSupportPayload.data.evidenceReadinessSummary?.tuiGatewayFixtureBackedCount, 0);
  ...
  for (const requirementId of [
    ...
    assert.equal(hermesSupportPayload.data.evidenceReadinessSummary?.tuiGatewayRequirementIds?.includes(requirementId), true);
    assert.equal(hermesSupportPayload.data.evidenceReadinessSummary?.tuiGatewayWrapperRequirementIds?.includes(requirementId), true);
    assert.equal(hermesSupportPayload.data.evidenceReadinessSummary?.tuiGatewayFixtureBackedRequirementIds?.includes(requirementId), false);
    assert.equal(hermesSupportPayload.data.evidenceReadinessSummary?.productionTransportRequirementIds?.includes(requirementId), true);
  }
  ...
  assert.equal(hermesSupportPayload.data.evidenceReadinessSummary?.writeBackContractRequirementIds?.includes("hermes.sessions.unpin.native_write_back_contract"), true);
  ...
  assert.equal(hermesSupportPayload.data.evidenceReadinessSummary?.statusCounts?.blocked_until_tui_gateway_wrapper_fixture, 4);
  assert.equal(hermesSupportPayload.data.evidenceReadinessSummary?.statusCounts?.blocked_until_upstream_contract, 12);
  ...
  assert.equal(hermesSupportPayload.data.finalPromotionReview?.externalPendingRequirementIds?.includes("hermes.providers.live_evidence"), true);
  assert.equal(hermesSupportPayload.data.finalPromotionReview?.externalPendingRequirementIds?.includes("hermes.auth.live_evidence"), true);
  ...
  assert.equal(hermesChannelReentry?.commandShape, "runtime hermes domain channels --json");
  assert.equal(hermesChannelReentry?.exactCommand, "claw runtime hermes domain channels --json");
  assert.equal(hermesChannelReentry?.preflightCommand, "claw runtime hermes support --json");
  assert.equal(hermesChannelReentry?.approvalScope, "read_only_redacted_live_domain_evidence_only");
  assert.equal(hermesChannelReentry?.evidenceSafetyPolicy, "redacted_values_only_in_commands_outputs_and_evidence");
  assert.equal(hermesChannelReentry?.expectedRedactedEvidence?.includes("no_plaintext_secrets_or_credentials"), true);
  assert.equal(hermesChannelReentry?.evidenceArtifactTemplate?.plaintextSecretLeak, false);
  assert.equal(hermesChannelReentry?.claimBlockedUntil, "approved_redacted_live_evidence_attached");
  assert.equal(hermesChannelReentry?.doNotRunWithoutApproval, true);
  assert.equal(hermesChannelReentry?.riskControls?.includes("read_only_first"), true);
  assert.equal(hermesSupportPayload.data.evidenceReentryPackets?.find((entry) => entry.requirementId === "hermes.providers.live_evidence")?.commandShape, "runtime hermes domain providers --json");
  assert.equal(hermesSupportPayload.data.evidenceReentryPackets?.find((entry) => entry.requirementId === "hermes.providers.live_evidence")?.exactCommand, "claw runtime hermes domain providers --json");
  assert.equal(hermesSupportPayload.data.evidenceReentryPackets?.find((entry) => entry.requirementId === "hermes.auth.live_evidence")?.commandShape, "runtime hermes domain auth --json");
  assert.equal(hermesSupportPayload.data.evidenceReentryPackets?.find((entry) => entry.requirementId === "hermes.auth.live_evidence")?.exactCommand, "claw runtime hermes domain auth --json");
  ...
  assert.equal(hermesCreateReentry?.exactCommand, "claw runtime hermes sessions create --title <approved-title> --confirm-runtime-write --gateway-url <approved-loopback-fixture-url> --json");
  assert.equal(hermesCreateReentry?.approvalScope, "approved_loopback_fixture_only_until_production_transport_policy");
  assert.equal(hermesCreateReentry?.evidenceSafetyPolicy, "redacted_values_only_in_commands_outputs_and_evidence");
  assert.equal(hermesCreateReentry?.expectedRedactedEvidence?.includes("no_plaintext_secrets_or_credentials"), true);
  assert.equal(hermesCreateReentry?.officialTransportSurface, "stdio_or_websocket_json_rpc");
  assert.deepEqual(hermesCreateReentry?.officialTransportClasses, ["stdio_json_rpc", "websocket_json_rpc"]);
  assert.equal(hermesCreateReentry?.officialTransportSource, "https://hermes-agent.nousresearch.com/docs/developer-guide/programmatic-integration");
  assert.equal(hermesCreateReentry?.productionTransportStatus, "blocked_until_production_transport_lifecycle_policy");
  assert.equal(hermesCreateReentry?.productionTransportBlocker, "approval_required_for_non_loopback_endpoint_and_lifecycle_management");
  ...
  assert.equal(await runCli(["runtime", "hermes", "commands", "--workspace", workspaceRoot, "--home-dir", hermesHome, "--json"], {
    stdout: hermesCommandsStdout.stream,
    stderr: captureStream().stream,
    cwd: process.cwd(),
  }), CLI_EXIT_OK);
  const hermesCommandsPayload = JSON.parse(hermesCommandsStdout.getOutput()) as {
    data: {
      runtimeId: string;
      authority?: string;
      ...
      jsonPortalCommandSet?: {
        ...
        domainCommandCount?: number;
        resourceCommandCount?: number;
        scopedReadCommandCount?: number;
        sessionActionCommandCount?: number;
        manifestDomainCount?: number;
        sessionActions?: string[];
        includesAllManifestDomains?: boolean;
        includesAllDocumentedSessionActions?: boolean;
        executableMatrixCommandCount?: number;
        promotionSignal?: boolean;
        supportClaim?: string;
        supportStage?: string;
  ...
  assert.equal(hermesCommandsPayload.data.jsonPortalCommandSet?.status, "guarded");
  assert.equal(hermesCommandsPayload.data.jsonPortalCommandSet?.totalCommandCount, 44);
  assert.equal(hermesCommandsPayload.data.jsonPortalCommandSet?.topLevelCommandCount, 5);
  assert.equal(hermesCommandsPayload.data.jsonPortalCommandSet?.domainCommandCount, manifest.requiredDomains.length);
  assert.equal(hermesCommandsPayload.data.jsonPortalCommandSet?.resourceCommandCount, manifest.requiredDomains.length);
  assert.equal(hermesCommandsPayload.data.jsonPortalCommandSet?.scopedReadCommandCount, 2);
  assert.equal(hermesCommandsPayload.data.jsonPortalCommandSet?.sessionActionCommandCount, 11);
  assert.deepEqual(hermesCommandsPayload.data.jsonPortalCommandSet?.sessionActions, ["list", "preview", "resolve", "history", "send", "inject", "abort", "create", "pin", "unpin", "conflicts"]);
  assert.equal(hermesCommandsPayload.data.jsonPortalCommandSet?.includesAllManifestDomains, true);
  assert.equal(hermesCommandsPayload.data.jsonPortalCommandSet?.includesAllDocumentedSessionActions, true);
  ...
  assert.equal(hermesCommandByName.get("runtime hermes sessions send --session-key <id> --message <text> --confirm-runtime-write")?.nativeWriteBackStatus, "blocked_until_tui_gateway_wrapper_fixture");
  assert.equal(hermesCommandByName.get("runtime hermes sessions send --session-key <id> --message <text> --confirm-runtime-write")?.nativeWriteBackBlockerClass, "direct_blocker");
  assert.equal(hermesCommandByName.get("runtime hermes sessions send --session-key <id> --message <text> --confirm-runtime-write")?.officialRuntimeWriteBackContractKnown, true);
  ...
  assert.equal([CLI_EXIT_OK, CLI_EXIT_DEGRADED].includes(await runCli(["runtime", "hermes", "domain", "sessions", "--workspace", workspaceRoot, "--home-dir", hermesHome, "--json"], {
    stdout: hermesDomainSessionsStdout.stream,
    stderr: captureStream().stream,
    cwd: process.cwd(),
  })), true);
  const hermesDomainSessionsPayload = JSON.parse(hermesDomainSessionsStdout.getOutput()) as {
    data: {
      runtimeId: string;
      domain: string;
      data: {
  ...
  const hermesActionByName = new Map((hermesDomainSessionsPayload.data.data.actionPolicy ?? []).map((entry) => [entry.action, entry]));
  assert.equal(hermesDomainSessionsPayload.data.runtimeId, "hermes");
  assert.equal(hermesDomainSessionsPayload.data.domain, "sessions");
  assert.equal(hermesDomainSessionsPayload.data.data.totalProjected, 1);
  assert.equal(hermesDomainSessionsPayload.data.data.supportContract?.writeBackAllowed, false);
  assert.equal(hermesDomainSessionsPayload.data.data.supportContract?.evidenceRequirements?.some((entry) => entry.id === "hermes.sessions.write_back_contract" && entry.blockerClass === "direct_blocker"), true);
  ...
  assert.equal(hermesDomainSessionsPayload.data.data.overlayState?.writeBackStatus, "blocked_until_official_runtime_pin_api");
  assert.equal(hermesDomainSessionsPayload.data.data.overlayState?.conflictPolicy, "no_silent_overwrite");
  ...
  for (const blockedAction of ["send", "inject"]) {
    ...
    assert.equal(blockedPayload.data.authority, "runtime");
    ...
    assert.equal(blockedPayload.data.commandShape, `runtime hermes sessions ${blockedAction} --session-key <id> --message <text> --confirm-runtime-write --json`);
  ...
  const hermesAbortPayload = JSON.parse(hermesAbortStdout.getOutput()) as { data: { action: string; status: string; writesRuntime: boolean; wouldWriteRuntime?: boolean; blockerClass?: string; safeDefault?: string; commandShape?: string; evidenceRequirementId?: string; reason?: string; officialContractRequired?: boolean; officialMethod?: string; evidenceReentryStatus?: string } };
  ...
  assert.equal(hermesAbortPayload.data.wouldWriteRuntime, true);
  ...
  assert.equal(hermesAbortPayload.data.commandShape, "runtime hermes sessions abort --session-key <id> --confirm-runtime-write --json");
  ...
  const hermesCreatePayload = JSON.parse(hermesCreateStdout.getOutput()) as { data: { action: string; status: string; writesRuntime: boolean; wouldWriteRuntime: boolean; blockerClass?: string; officialContractRequired?: boolean; officialContractKnown?: boolean; officialMethod?: string; fixtureRequired?: boolean; safeDefault?: string; userVisibleContract?: string; claimEffect?: string; commandShape?: string; evidenceRequirementId?: string; createPlan?: { requested?: { title?: string }; rejectedFallbacks?: string[]; requiredEvidence?: string[]; writeBackStatus?: string; acceptedContracts?: string[]; nextContract?: { fixtureRequirement?: string; officialMethod?: string } } } };
  ...
  assert.equal(hermesCreatePayload.data.commandShape, "runtime hermes sessions create --title <title> --confirm-runtime-write --json");
