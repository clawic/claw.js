...
test("search embeddings status rejects provider embedding models", async () => {
  const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "claw-search-embedding-status-gate-"));
