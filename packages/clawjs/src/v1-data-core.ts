...
export function usage(binName: string, group: string): string {
  ...
  switch (group) {
    ...
    case "agents": return `Usage: ${binName} agents list|get|upsert|delete|schema|evaluate-access|route-check|resolve-external-identity|project-support-inbox|memory-check [--json]`;
