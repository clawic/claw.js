...
test("runCli rejects invalid app manifest and permissions JSON before persistence", async () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "clawjs-cli-v2-apps-json-"));
  await withPatchedEnv({
    CLAW_HOME: path.join(tempRoot, "home"),
    CLAW_DATA_DIR: tempRoot,
    CLAW_DB_PATH: undefined,
    ...
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "clawjs-cli-v2-apps-json-cwd-"));
    const cases: Array<{ flag: string; code: string }> = [
      { flag: "--manifest", code: "invalid_app_manifest_json" },
      { flag: "--permissions", code: "invalid_app_permissions_json" },
    ];
    ...
    for (const testCase of cases) {
      ...
      assert.equal(payload.error.status, "USAGE");
      assert.equal(payload.meta.canonicalCommand, "apps");
    }
