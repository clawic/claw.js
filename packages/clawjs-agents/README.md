...
Agent identity, composition and lifecycle for ClawJS. The package is filesystem-first: agents, personalities, skill collections and connections live under `~/.clawjs/` (override with `CLAWIX_CLAW_HOME`) and `AgentStoreFS` is a thin reader / writer on top.
