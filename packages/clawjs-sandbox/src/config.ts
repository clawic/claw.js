...
export function loadSandboxConfig(overrides: Partial<SandboxServiceConfig> = {}): SandboxServiceConfig {
  ...
  const dataDir = overrides.dataDir ?? process.env.SANDBOX_DATA_DIR ?? path.join(cwd, ".data");
  return {
    ...
    dbPath: overrides.dbPath ?? process.env.SANDBOX_DB_PATH ?? path.join(dataDir, "sandbox.sqlite"),
    dataDir,
