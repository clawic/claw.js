...
CREATE TABLE IF NOT EXISTS marketplace_root_keys (
  id              TEXT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS marketplace_device_keys (
  id              TEXT PRIMARY KEY,
  root_key_id     TEXT NOT NULL REFERENCES marketplace_root_keys(id) ON DELETE CASCADE,
  pubkey          BLOB NOT NULL UNIQUE,
...
CREATE INDEX IF NOT EXISTS idx_marketplace_device_keys_root ON marketplace_device_keys(root_key_id);

CREATE TABLE IF NOT EXISTS marketplace_role_keys (
  id              TEXT PRIMARY KEY,
  root_key_id     TEXT NOT NULL REFERENCES marketplace_root_keys(id) ON DELETE CASCADE,
  pubkey          BLOB NOT NULL UNIQUE,
  encrypted_priv  BLOB NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_marketplace_role_keys_root ON marketplace_role_keys(root_key_id);
CREATE INDEX IF NOT EXISTS idx_marketplace_role_keys_vertical ON marketplace_role_keys(vertical);

CREATE TABLE IF NOT EXISTS marketplace_cross_role_attestations (
  id              TEXT PRIMARY KEY,
  root_key_id     TEXT NOT NULL REFERENCES marketplace_root_keys(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS marketplace_intents (
  id                  TEXT PRIMARY KEY,
  intent_id_hash      BLOB NOT NULL UNIQUE,
  side                TEXT NOT NULL CHECK (side IN ('offer', 'want')),
  role_key_id         TEXT REFERENCES marketplace_role_keys(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_marketplace_intents_vertical ON marketplace_intents(vertical, status);
CREATE INDEX IF NOT EXISTS idx_marketplace_intents_role ON marketplace_intents(role_key_id);
CREATE INDEX IF NOT EXISTS idx_marketplace_intents_side ON marketplace_intents(side, status);

CREATE TABLE IF NOT EXISTS marketplace_intent_observations (
  id              TEXT PRIMARY KEY,
  intent_id       TEXT NOT NULL REFERENCES marketplace_intents(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_marketplace_intent_obs_intent ON marketplace_intent_observations(intent_id);

CREATE TABLE IF NOT EXISTS marketplace_peer_levels (
  id                  TEXT PRIMARY KEY,
  my_role_key_id      TEXT NOT NULL REFERENCES marketplace_role_keys(id) ON DELETE CASCADE,
  peer_pubkey         BLOB NOT NULL,
  current_level       INTEGER NOT NULL DEFAULT 0 CHECK (current_level BETWEEN 0 AND 5),
  intent_id           TEXT REFERENCES marketplace_intents(id) ON DELETE SET NULL,
  proofs_json         TEXT,
  last_updated_at     TEXT NOT NULL DEFAULT (datetime('now')),
  UNIQUE (my_role_key_id, peer_pubkey, intent_id)
);

CREATE INDEX IF NOT EXISTS idx_marketplace_peer_levels_role ON marketplace_peer_levels(my_role_key_id);

CREATE TABLE IF NOT EXISTS marketplace_inbound_messages (
  id                  TEXT PRIMARY KEY,
  recipient_role_key_id TEXT NOT NULL REFERENCES marketplace_role_keys(id) ON DELETE CASCADE,
  sender_pubkey       BLOB NOT NULL,
  thread_id           BLOB,
  in_reply_to         BLOB,
  intent_id_ref       TEXT REFERENCES marketplace_intents(id) ON DELETE SET NULL,
  kind                TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_marketplace_inbound_recipient ON marketplace_inbound_messages(recipient_role_key_id, received_at DESC);
CREATE INDEX IF NOT EXISTS idx_marketplace_inbound_thread ON marketplace_inbound_messages(thread_id);

CREATE TABLE IF NOT EXISTS marketplace_outbound_messages (
  id                  TEXT PRIMARY KEY,
  sender_role_key_id  TEXT NOT NULL REFERENCES marketplace_role_keys(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_marketplace_outbound_sender ON marketplace_outbound_messages(sender_role_key_id, sent_at DESC);

CREATE TABLE IF NOT EXISTS marketplace_match_receipts (
  id                  TEXT PRIMARY KEY,
  receipt_hash        BLOB NOT NULL UNIQUE,
  my_role_key_id      TEXT NOT NULL REFERENCES marketplace_role_keys(id) ON DELETE CASCADE,
  peer_role_pubkey    BLOB NOT NULL,
  offer_intent_id     TEXT REFERENCES marketplace_intents(id) ON DELETE SET NULL,
  want_intent_id      TEXT REFERENCES marketplace_intents(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_marketplace_match_receipts_role ON marketplace_match_receipts(my_role_key_id);
CREATE INDEX IF NOT EXISTS idx_marketplace_match_receipts_status ON marketplace_match_receipts(status);

CREATE TABLE IF NOT EXISTS marketplace_vouches_outbound (
  id                  TEXT PRIMARY KEY,
  voucher_role_key_id TEXT NOT NULL REFERENCES marketplace_role_keys(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS marketplace_vouches_inbound (
  id                  TEXT PRIMARY KEY,
  my_role_key_id      TEXT NOT NULL REFERENCES marketplace_role_keys(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS marketplace_ratings (
  id                  TEXT PRIMARY KEY,
  match_receipt_id    TEXT NOT NULL REFERENCES marketplace_match_receipts(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS marketplace_known_brokers (
  id                  TEXT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS marketplace_dht_records_cache (
  key                 BLOB NOT NULL,
