...
const baseHermesAdapter = createSimpleRuntimeAdapter({
  ...
  capabilityDeclarations: {
    ...
    auth: { supported: true, status: "degraded", strategy: "config", limitations: ["Hermes auth state may come from the configured Hermes auth store and environment-backed secrets."] },
    models: { supported: true, status: "ready", strategy: "cli" },
    scheduler: { supported: true, status: "ready", strategy: "native", limitations: ["Scheduler inventory is read from the configured Hermes scheduler store when available and falls back to runtime probing."] },
    memory: { supported: true, status: "ready", strategy: "bridge", limitations: ["Memory inventory is mapped from the configured Hermes memory store and session storage."] },
    ...
    skills: { supported: true, status: "ready", strategy: "native", limitations: ["Skill inventory is read from the configured Hermes skills inventory and does not expose all Hermes skill metadata yet."] },
