...
export function createMockRuntimeProbeStatus(input: {
  ...
  return {
    ...
    capabilityMap: buildRuntimeCapabilityMap({
      ...
      session_cli: { supported: true, status: input.cliAvailable ? "ready" : "error", strategy: "cli" },
      session_gateway: { supported: input.gatewayAvailable, status: input.gatewayAvailable ? "ready" : "unsupported", strategy: input.gatewayAvailable ? "gateway" : "unsupported" },
