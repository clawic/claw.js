// `real-estate/v1` plugin for the Clawix mp/* protocol.
//
// This package re-exports the canonical real-estate schema that ships inside
// `@clawjs/mp/verticals` and wraps it as a `MpVerticalPlugin` so it can be
// loaded into a `VerticalRegistry` alongside the core verticals.
//
// We keep the schema source in `@clawjs/mp/verticals` for now to avoid
// duplicating large field tables. A future release can flip this to be the
// authoritative location and have `@clawjs/mp` re-export from here instead.
