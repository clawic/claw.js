...
export const clawCapabilityFiches: readonly ClawCapabilityFiche[] = [
  ...
  fiche({
    ...
    inputs: ["remote client request", "technical tenant and workspace context from Relay", "connector session command"],
    ...
    permissions: ["Relay auth", "connector authority for workspace scope", "remote-safe session classification"],
    ...
    limits: ["Remote-safe projection only", "No plaintext secrets", "Connector remains the authority source for workspace-scoped remote access"],
  ...
  customAppFiche({
    ...
    inputs: ["claw.system.telemetry.metrics.request.v1"],
    outputs: ["claw.system.telemetry.metrics.v1"],
    storage: ["claw.systemTelemetry"],
    limits: ["Read-only", "Local-only Relay classification"],
  }),
  customAppFiche({
    ...
    resourceNodes: ["claw.systemTelemetry"],
    routeIds: ["system.telemetryAgentContext"],
    cli: ["claw system widgets --json"],
    serviceApi: ["claw.api.system.widgets"],
    mcp: ["system.widgets"],
    relay: ["local-only"],
    hostBridge: ["clawix.bridge.local"],
    inputs: ["claw.system.telemetry.widgets.request.v1"],
    outputs: ["claw.system.telemetry.widgets.v1"],
    storage: ["claw.systemTelemetry"],
    limits: ["Read-only", "Local-only Relay classification"],
  }),
  customAppFiche({
    ...
    summary: "Read provider catalog entries, credential projection contracts, availability modes, and provider-plan requirements.",
    operation: "read",
    resourceNodes: ["claw.systemTelemetry", "claw.systemTelemetry.contextProviders"],
    routeIds: ["system.telemetryAgentContext"],
    cli: ["claw system providers --json"],
    serviceApi: ["claw.api.system.providers"],
    mcp: ["system.providers"],
    relay: ["local-only"],
    hostBridge: ["clawix.bridge.local"],
    inputs: ["claw.system.telemetry.providers.request.v1"],
    outputs: ["claw.system.telemetry.providers.v1"],
    storage: ["claw.systemTelemetry"],
    limits: ["Read-only", "Credentials remain redacted references", "Local-only Relay classification"],
  }),
  customAppFiche({
    id: "system.telemetry.control.plan",
    system: "system",
    title: "System telemetry control plan",
    summary: "Plan system telemetry control actions through signed-host approval without executing OS or hardware mutations directly.",
    operation: "action",
    resourceNodes: ["claw.systemTelemetry", "claw.host.signed"],
    routeIds: ["system.telemetrySignedHostControl"],
    cli: ["claw system controls plan --json"],
    serviceApi: ["claw.api.system.controls.plan"],
    mcp: ["system.controls.plan"],
    relay: ["local-only"],
    hostBridge: ["signed-host system telemetry broker"],
    inputs: ["claw.system.telemetry.controlPlan.request.v1"],
    outputs: ["claw.system.telemetry.controlPlan.v1"],
    storage: ["claw.systemTelemetry", "claw.host.audit"],
    permissions: ["approval required", "host audit receipt"],
    errorStates: ["approval denied", "signed host unavailable", "control validation failed"],
    limits: ["Plan-first", "No direct public native control", "Live execution remains host-gated"],
    gaps: [externalPendingGap],
  }),
  ...
  customAppFiche({ id: "jobs.stream", system: "jobs", title: "Jobs stream", summary: "Read runtime job event snapshots through SDK/host adapters without starting or cancelling work.", operation: "read", resourceNodes: ["claw.runtime.agent", "claw.agents.runs"], routeIds: [], cli: ["blocked until a public jobs stream CLI contract exists"], serviceApi: ["claw.api.runtime.jobs"], mcp: ["clawjs.custom_app_sdk metadata-only contract projection"], relay: ["local-only"], hostBridge: ["clawix.bridge.local"], permissions: ["local-wide read", "shared redaction policy"], limits: ["No job mutation", "Public CLI remains blocked"], gaps: [{ area: "registered route", status: "deferred", reason: "Jobs stream is a custom-app stream projection; no dedicated route graph entry is required in the critical slice." }, { area: "public CLI", status: "blocked", reason: "Internal runtime job streams are removed from the public CLI and must not be advertised as a jobs stream surface." }] }),
  customAppFiche({ id: "jobs.start", system: "jobs", title: "Jobs start", summary: "Start an allowlisted runtime job through host approval, audit, and runtime jobs API.", operation: "action", resourceNodes: ["claw.runtime.agent", "claw.agents.runs"], routeIds: [], cli: ["blocked until a public jobs mutation CLI contract exists"], serviceApi: ["claw.api.runtime.jobs"], mcp: ["clawjs.custom_app_sdk metadata-only contract projection"], relay: ["local-only"], hostBridge: ["clawix.bridge.local"], permissions: ["approval required", "host audit receipt"], errorStates: ["approval denied", "runtime unavailable", "job validation failed"], limits: ["Allowlisted runtime job kinds only", "Public CLI remains blocked"], gaps: [{ area: "registered route", status: "deferred", reason: "Jobs mutation remains a host-approved custom-app projection until the runtime jobs route graph is promoted." }, { area: "public CLI", status: "blocked", reason: "Internal runtime job mutation is not exposed as a public CLI surface." }] }),
  customAppFiche({ id: "jobs.cancel", system: "jobs", title: "Jobs cancel", summary: "Cancel a runtime job through host approval, audit, and runtime jobs API.", operation: "action", resourceNodes: ["claw.runtime.agent", "claw.agents.runs"], routeIds: [], cli: ["blocked until a public jobs mutation CLI contract exists"], serviceApi: ["claw.api.runtime.jobs"], mcp: ["clawjs.custom_app_sdk metadata-only contract projection"], relay: ["local-only"], hostBridge: ["clawix.bridge.local"], permissions: ["approval required", "host audit receipt"], errorStates: ["approval denied", "runtime unavailable", "job not found"], limits: ["Cancels runtime jobs only", "Public CLI remains blocked"], gaps: [{ area: "registered route", status: "deferred", reason: "Jobs mutation remains a host-approved custom-app projection until the runtime jobs route graph is promoted." }, { area: "public CLI", status: "blocked", reason: "Internal runtime job mutation is not exposed as a public CLI surface." }] }),
