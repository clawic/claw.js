...
const remoteRouteContractDefinitions: RemoteRouteContract[] = [
  ...
  {
    schemaVersion: 1,
    routeId: "sync.sessions",
    layer: "sync",
    capability: "session metadata, transcript, and conversation-state synchronization",
    localContractRefs: ["claw sync manifest --driver sessions", "claw sessions", "docs/relay.md"],
    remoteEntryPoints: ["POST /v1/sync/plan", "POST /v1/sync/applications"],
    decisionIds: ["sync_substrate", "sync_lateral_domains", "remote_api_shape"],
    parityRequired: true,
    parallelApiAllowed: false,
    writes: false,
  },
  {
    ...
    layer: "sync",
    capability: "drive and file synchronization",
    localContractRefs: ["claw sync manifest --driver drive_files", "docs/relay.md"],
    remoteEntryPoints: ["POST /v1/sync/plan", "POST /v1/sync/applications"],
  ...
  },
  {
    ...
    routeId: "sync.blobs",
    layer: "sync",
    capability: "blob synchronization",
    localContractRefs: ["claw sync manifest --driver blobs", "docs/relay.md"],
    remoteEntryPoints: ["POST /v1/sync/plan", "POST /v1/sync/applications"],
    decisionIds: ["sync_substrate", "sync_lateral_domains", "client_cache_policy"],
    parityRequired: true,
    parallelApiAllowed: false,
  ...
  },
  {
    ...
    layer: "sync",
    capability: "search index synchronization and rebuild-state propagation",
    localContractRefs: ["claw sync manifest --driver search_index", "claw search", "docs/search.md"],
    remoteEntryPoints: ["POST /v1/sync/plan", "POST /v1/sync/applications"],
    decisionIds: ["sync_substrate", "sync_lateral_domains", "remote_surface_parity"],
    parityRequired: true,
    parallelApiAllowed: false,
    writes: false,
  },
  {
    ...
    layer: "sync",
    capability: "SQLite tables and partial SQLite synchronization",
    localContractRefs: ["claw sync manifest --driver sqlite_tables", "claw sync manifest --driver sqlite_partial"],
    remoteEntryPoints: ["POST /v1/sync/plan", "POST /v1/sync/applications"],
  ...
  {
    schemaVersion: 1,
    routeId: "sync.sidecars",
    layer: "sync",
    capability: "sidecar database synchronization",
    localContractRefs: ["claw sync manifest --driver sidecar", "docs/relay.md"],
    remoteEntryPoints: ["POST /v1/sync/plan", "POST /v1/sync/applications"],
    decisionIds: ["sync_substrate", "sync_lateral_domains", "conflict_default"],
    parityRequired: true,
    parallelApiAllowed: false,
  ...
  {
    schemaVersion: 1,
    routeId: "sync.agentConfig",
    layer: "sync",
    capability: "agent configuration synchronization",
    localContractRefs: ["claw sync manifest --driver agent_config", "docs/relay.md"],
    remoteEntryPoints: ["POST /v1/sync/plan", "POST /v1/sync/applications"],
    decisionIds: ["sync_substrate", "sync_lateral_domains", "agent_service_model"],
    parityRequired: true,
    parallelApiAllowed: false,
    writes: false,
  },
  {
    schemaVersion: 1,
    routeId: "sync.workspaceState",
    layer: "sync",
    capability: "workspace state synchronization",
    localContractRefs: ["claw sync manifest --driver workspace_state", "docs/relay.md"],
    remoteEntryPoints: ["POST /v1/sync/plan", "POST /v1/sync/applications"],
    decisionIds: ["sync_substrate", "sync_lateral_domains", "headless_host_model"],
    parityRequired: true,
    parallelApiAllowed: false,
    writes: false,
  },
