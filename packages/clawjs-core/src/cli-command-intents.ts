...
const registryEntries: ClawCliCommandIntentEntry[] = [
  ...
  intent("cmd_intent_runtime_summary", "runtime summary", "Read a runtime-scoped ecosystem summary without executing runtime writes.", "covered", {
    mappedCommand: "runtime <runtime-id> summary",
    relatedCommands: ["runtime", "summary", "openclaw", "codex", "hermes"],
    risk: ["local_read"],
    evidence: ["`runtime <runtime-id> summary --json` returns support, capability, domain, session, and resource aggregates for the selected runtime only."],
    nextSteps: ["Run `claw runtime hermes summary --json` to inspect the Hermes runtime-lens source summary."],
  }),
  intent("cmd_intent_runtime_status", "runtime status", "Read installed, CLI, gateway, capability, version, and diagnostic status for one runtime.", "covered", {
    mappedCommand: "runtime <runtime-id> status",
    relatedCommands: ["runtime", "status", "openclaw", "codex", "hermes"],
    risk: ["local_read"],
    evidence: ["`runtime <runtime-id> status --json` is the read-only adapter status probe used before support-claim promotion."],
    nextSteps: ["Run `claw runtime hermes status --json` with `--home-dir` or `--binary-path` when validating an isolated Hermes fixture."],
  }),
  intent("cmd_intent_runtime_commands", "runtime commands", "List the Claw CLI command matrix for one runtime portal.", "covered", {
    mappedCommand: "runtime <runtime-id> commands",
    relatedCommands: ["runtime", "commands", "openclaw", "codex", "hermes"],
    risk: ["local_read"],
    evidence: ["`runtime <runtime-id> commands --json` exposes runtime-scoped command rows, write policy, and blocked/local-overlay dispositions."],
    nextSteps: ["Run `claw runtime hermes commands --json` to inspect blocked Hermes write/control rows without executing them."],
  }),
  intent("cmd_intent_runtime_session", "runtime session", "Inspect the session transport descriptor for one runtime.", "covered", {
    mappedCommand: "runtime <runtime-id> session",
    relatedCommands: ["runtime", "session", "sessions", "openclaw", "codex", "hermes"],
    risk: ["local_read"],
    evidence: ["`runtime <runtime-id> session --json` returns the selected runtime's session transport, gateway support, persistence, and session-path descriptor."],
    nextSteps: ["Run `claw runtime hermes session --json` to verify Hermes gateway/CLI transport metadata."],
  }),
  intent("cmd_intent_runtime_workspace", "runtime workspace", "Inspect the managed workspace contract and canonical paths for one runtime.", "covered", {
    mappedCommand: "runtime <runtime-id> workspace",
    relatedCommands: ["runtime", "workspace", "openclaw", "codex", "hermes"],
    risk: ["local_read"],
    evidence: ["`runtime <runtime-id> workspace --json` returns managed files and canonical paths for the selected runtime without mutating runtime-owned state."],
    nextSteps: ["Run `claw runtime hermes workspace --json` with a fixture workspace when validating Hermes managed-file projection."],
  }),
  ...
  intent("cmd_intent_runtime_sessions_send", "runtime sessions send", "Send a message through a native runtime session when an official write path exists.", "covered", {
    mappedCommand: "runtime <runtime-id> sessions send",
    relatedCommands: ["runtime", "sessions", "openclaw", "codex", "hermes"],
    risk: ["local_write", "external_service"],
    evidence: ["OpenClaw `sessions send` is exposed through the official gateway wrapper; Codex/Hermes return structured blocked responses until official send contracts and fixtures exist."],
    nextSteps: ["Run `claw runtime <runtime-id> sessions send --session-key <id> --message <text> --json` first to inspect the non-mutating confirmation or blocked contract; add `--confirm-runtime-write` only for an intentional supported runtime write."],
  }),
  intent("cmd_intent_runtime_sessions_inject", "runtime sessions inject", "Inject a system note through a native runtime session when an official write path exists.", "covered", {
    mappedCommand: "runtime <runtime-id> sessions inject",
    relatedCommands: ["runtime", "sessions", "openclaw", "codex", "hermes"],
    risk: ["local_write", "external_service"],
    evidence: ["OpenClaw `sessions inject` is exposed through the official gateway wrapper; Codex/Hermes return structured blocked responses until official inject contracts and fixtures exist."],
    nextSteps: ["Run `claw runtime <runtime-id> sessions inject --session-key <id> --message <text> --json` first to inspect the non-mutating confirmation or blocked contract; add `--confirm-runtime-write` only for an intentional supported runtime write."],
  }),
  intent("cmd_intent_runtime_sessions_abort", "runtime sessions abort", "Abort a native runtime session run when an official control path exists.", "covered", {
    mappedCommand: "runtime <runtime-id> sessions abort",
    relatedCommands: ["runtime", "sessions", "openclaw", "codex", "hermes"],
    risk: ["local_write", "external_service"],
    evidence: ["OpenClaw `sessions abort` is exposed through the official gateway wrapper; Codex/Hermes return structured blocked responses until official abort contracts and fixtures exist."],
    nextSteps: ["Run `claw runtime <runtime-id> sessions abort --session-key <id> --json` first to inspect the non-mutating confirmation or blocked contract; add `--confirm-runtime-write` only for an intentional supported runtime control action."],
  }),
