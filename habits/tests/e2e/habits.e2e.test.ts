...
import { buildHabitsApp, HabitsClient } from "@clawjs/habits";

test("habits CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "habits-e2e-"));
  const dbPath = path.join(tmpDir, "habits.sqlite");
  ...
  try {
    ...
    const client = new HabitsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "habits");
