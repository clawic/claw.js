...
import { buildYearReviewsApp, YearReviewsClient } from "@clawjs/year-reviews";

test("year-reviews CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "year-reviews-e2e-"));
  const dbPath = path.join(tmpDir, "year-reviews.sqlite");
  ...
  try {
    ...
    const client = new YearReviewsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "year-reviews");
