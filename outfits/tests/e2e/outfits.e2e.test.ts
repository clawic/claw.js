...
import { buildOutfitsApp, OutfitsClient } from "@clawjs/outfits";

test("outfits CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "outfits-e2e-"));
  const dbPath = path.join(tmpDir, "outfits.sqlite");
  ...
  try {
    ...
    const client = new OutfitsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "outfits");
