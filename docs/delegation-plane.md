...
`delegation-plane/` is a standalone control plane for durable agent delegation.
...
```bash
npm --prefix delegation-plane install
npm --prefix delegation-plane run build
npm --prefix delegation-plane run start
```
...
```bash
npm --prefix delegation-plane run cli -- graph create --objective "Ship the release plan"
npm --prefix delegation-plane run cli -- graph list
npm --prefix delegation-plane run cli -- graph inspect <graphId> --tree
```

Register a worker and claim work:

```bash
npm --prefix delegation-plane run cli -- worker register --workerId local-1
npm --prefix delegation-plane run cli -- worker claim local-1
```

Operate stuck work:

```bash
npm --prefix delegation-plane run cli -- stuck
npm --prefix delegation-plane run cli -- node retry <nodeId>
npm --prefix delegation-plane run cli -- node cancel <nodeId>
```
