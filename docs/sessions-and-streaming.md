---
title: Sessions and Structured Stream Events
description: Legacy compatibility alias for the canonical Sessions guide.
---

# Sessions and Structured Stream Events

This page is kept as a compatibility alias for older links. The
canonical reference now lives in [Sessions](/sessions).

## Quick Reference

- Session transcripts live under `.clawjs/sessions/<session-id>.jsonl`
- `claw.sessions` exposes session CRUD, title generation, session
  search, structured event streaming, and raw chunk streaming
- `claw sessions stream --events` emits the same normalized event model
  as the Node API

## Canonical Pages

- [Sessions](/sessions) for the current session and transport reference
- [API Reference](/api) for the instance-level method inventory
- [CLI](/cli) for command-line flags and usage examples
