...
| Workspace wording with access-control language | Workspace is an isolated context but may be phrased as if it grants access. | Workspace is scope/context; grants are explicit. | Reword scope-derived access phrasing. Verify code does not infer access from workspace membership alone. | Effective-access tests cover workspace scope with and without grants; vocabulary guard blocks scope-as-access patterns. |
| Project wording with access-control language | Project is a collaborable work scope, but folder/project wording may imply access. | Project identity is stable `projectId`; folder path/name is locator only; direct grants at project scope are explicit. | Reword project-derived access language; audit path-keyed project/sidebar/session behavior separately. | Tests cover project rename, folder move/copy, duplicate id, explicit grant at project scope, and no authority from folder path. |
| Agent wording with access-control language | Agent can be a principal but does not automatically receive access. | Effective access comes from grants, assignments, leases, restrictions, and actor delegation. | Reword agent-derived access phrases where they mean grants for an agent or effective access. | Tests cover agent acting on behalf of a principal and delegation intersection. |
| Relay wording with access-control language | Relay may broker remote flows but cannot own native, secret, grant, or approval authority. | Relay is transport/control-plane brokering; local API and authority remain framework/host governed. | Reword Relay-derived access phrases; keep technical isolation only where documented. | `claw inspect show claw.relay --json` and route tests show Relay brokers/exposes/consumes but does not grant access. |
| Sync wording with access-control language | Sync can look like access transfer. | Sync reconciles under explicit manifest, conflict, cache, and authority-handoff contracts; it does not grant access. | Reword sync-derived access language; audit sync routes for explicit handoff receipts and conflict policy. | Sync route tests prove authorization is consumed from explicit contracts; guard blocks replication-as-access patterns. |
| Route wording with access-control language | Route graph paths may be mistaken for permission paths. | Route is graph traversal with explicit edge semantics; it does not grant access. | Reword graph-path permission language. Keep `route` only as graph/transport concept. | Surface-route guard and conceptual guard both pass graph-path-as-access negative fixtures. |
...
| Connector approval-bound grant contract | Connector control-plane API exposes `ConnectorApprovalGrant` and `approvalGrantId`. | If retained, the type must mean a scoped grant record tied to approval evidence; approval itself remains a review/consent gate and not durable access. | Decide whether to document the identifier as connector-domain compatibility, migrate to split approval receipt and scoped grant names, or provide old/new schema aliases. | Connector control-plane tests prove missing, expired, or out-of-scope records fail closed; vocabulary docs state why this exported identifier is permitted or how it migrates. |
