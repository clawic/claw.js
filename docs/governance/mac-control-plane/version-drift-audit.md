...
| --- | ---: | --- | --- | --- | --- |
| `mixed` | 3 | Supported for Wi-Fi read/list and central permission planning. | No V1 drift recorded. | No V1 drift recorded. | Keep broker-owned composition; do not expose native calls outside signed host. |
...
| `manual` | 12 | Atlas-only planned entries; not executable. | No executable claim. | No executable claim. | Re-audit official backend before changing coverage state. |
...
| `mac.audio.volume` | `manual` | planned | Atlas-only; no executable claim until audio-device strategy is audited. |
| `mac.display.brightness` | `manual` | planned | Atlas-only; no executable claim until display strategy is audited. |
