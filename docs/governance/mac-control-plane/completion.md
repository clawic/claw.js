...
`docs/governance/mac-control-plane/source-audit.md` as `MCQ-001` through
`MCQ-018`.
...
| MCQ-011 | implemented | First-slice and atlas-only verbs map to stable internal capability IDs, direct CLI roots, and documented model-facing semantics. Evidence: `docs/governance/mac-control-plane/verb-audit.md`, `MAC_CAPABILITY_ATLAS`, core atlas coverage tests, and the Mac goal verifier requiring every atlas capability id and canonical CLI usage in the verb audit. |
...
3. The `Structured Prompt Review` in
   `docs/governance/mac-control-plane/source-audit.md` is re-read against the
