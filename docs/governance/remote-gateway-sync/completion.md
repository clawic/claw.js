...
[Remote Gateway And Sync Source Decision Audit](./source-audit.md)
...
| RQ-015 | `client_cache_policy` | external_pending | `RemoteClientCacheSnapshot` enforces encrypted TTL cache, no secrets, no authoritative state, no plaintext payload, and fail-closed `physicalClientStorageVerified` evidence gating. | Real client storage validation remains `physical_client_storage` `EXTERNAL PENDING`. |
...
docs/governance/remote-gateway-sync/source-review.json --external-validation-file
docs/governance/remote-gateway-sync/external-validation-evidence.json --json`, overlays the
...
docs/governance/remote-gateway-sync/source-review.json --external-validation-file
docs/governance/remote-gateway-sync/external-validation-evidence.json --json` returned
...
docs/governance/remote-gateway-sync/external-validation-evidence.json --json` kept all 13
requirements blocked with zero clearable requirements, while `claw remote
closure-gate --source-qa-review-file
docs/governance/remote-gateway-sync/source-review.json --external-validation-file
docs/governance/remote-gateway-sync/external-validation-evidence.json --json` confirmed
...
docs/governance/remote-gateway-sync/source-review.json --external-validation-file
docs/governance/remote-gateway-sync/external-validation-evidence.json --json`; it returned
...
3. `RemoteExternalPendingRegister` contains every remaining physical/provider
   ...
   `docs/governance/remote-gateway-sync/external-validation-evidence.json` artifact must
   ...
   Individual signed receipt commands must also fail closed: physical/provider
   verification booleans do not clear pending rows unless the command or Relay
   payload carries an approved run reference and physical evidence reference.
