...
`docs/governance/remote-gateway-sync/source-review.json` records the current
...
| RQ-004 | `topology_priority` | Personal devices and headless/server deployments use the same architecture. | Personal mesh, server host, headless host, multi-tenant Gateway, and provider/device E2E topology targets share the same conformance contract. |
...
| RQ-009 | `offline_behavior` | Interactive commands fail fast; sync uses queued planning, cursors, changelogs, and reconciliation. | `RemoteOfflineCommandResult`, `claw remote offline-command`, and Relay `/v1/remote/offline-command` prove interactive remote calls fail fast with `enqueued: false`; `buildSyncPlan` returns dry-run actions, changes, conflicts, and next cursors with `writes: false`; `RemoteSyncStateStore` persists opt-in local queue/reconciliation state through `claw sync run --state-dir &lt;dir&gt; --queue true` and `claw sync reconcile`, with optional Ed25519 Coordinator signatures. |
...
| RQ-013 | `sync_substrate` | Manifests and changelogs govern sync without constraining the physical driver. | `SyncDriverCatalog`, `claw sync drivers`, and Relay `/v1/sync/drivers` enumerate the manifest-backed, changelog-backed, authority-scoped driver matrix; `SyncDriverApplicationReceipt`, `claw sync apply`, and Relay `/v1/sync/applications` bind reconciled changes to one driver while physical driver execution remains explicit. |
...
| RQ-022 | `sync_lateral_domains` | Sync must cover lateral domains: skills, memory, drive/files, sessions, agent config, workspace state, databases, sidecars, blobs, and indexes. | `SyncDriverCatalog` covers skills, memory/user-model, sessions, drive/files, blobs, full SQLite, partial SQLite, sidecars, search indexes, agent config, and workspace state with required Sync routes and no-write physical-driver gating. |
