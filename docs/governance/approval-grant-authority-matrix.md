...
owner that already decides the domain. New approval or grant work must extend
the owning domain instead of creating a parallel registry.

| Domain | Canonical owner | Grant or approval type | Inspection route | Audit path | Fail-closed behavior |
| --- | --- | --- | --- | --- | --- |
...
| Publishing and irreversible actions | Domain owner plus No Irreversible Data Loss governance | Publication approval, destructive action approval, rollback evidence | Domain CLI/API and no-irreversible-data-loss checks | Domain audit plus recovery/rollback evidence | Hard delete or external mutation blocks unless classified, recoverable, and explicitly approved. |
| Report/publication workflows | Report governance and connector owner | Human publication approval and brokered credential lease | Report governance docs and connector decision | Report artifact metadata plus connector audit | Preview-only until host approval and connector-scoped credential lease are present. |
...
- A grant is valid only inside its owning domain and scope.
...
- New domains must add one row here only after the owner, inspection route,
