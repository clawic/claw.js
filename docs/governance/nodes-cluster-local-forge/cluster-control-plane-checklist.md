...
- [ ] blocked: Run `npm run test:docs`, route checks, and affected remote/sync/governance tests after implementation. Blocker: latest `npm run test:docs` rerun fails only on unrelated `persistent_surface_database_literal` findings in `packages/clawjs-database/src/database-query-plan.test.ts:153` and `:161`. Reentry: rerun after inherited failure is resolved or accepted as separate cleanup. Evidence so far: affected remote/sync/governance tests, route checks, docs build, storage-boundary, source-audit, discoverability, ADR coverage, and surface checks passed before the inherited guard failure.
