...
| LC-006 | Audience limits | Strong limits | Terms, Disclaimer, Safety, README, and EULA state personal/local assistive use and no professional replacement. | App-store and binary packaging text review is `EXTERNAL PENDING` until the exact channel exists. |
...
| LC-011 | Terms acceptance | Initial clickwrap | Clawix legal state and tests cover current acceptance persistence. | User-facing signed app validation is tracked as `EXTERNAL PENDING` by the Clawix ledger. |
...
| LC-014 | Support data | Manual opt-in | Privacy and Clawix legal settings require manual support diagnostics opt-in; Clawix rescue diagnostics tests prove exported support context is redacted and marked `explicit_approval_only` before any external submission. | Signed support UI smoke is `EXTERNAL PENDING` until an approved current app lane runs. |
| LC-015 | Third-party data | Limited and consented | Terms/Privacy say third-party sensitive data requires lawful basis or consent and minimization; the legal release verifier now requires the public mock seed to declare synthetic placeholder data and rejects non-reserved email domains in that seed. | Future examples and demos must stay synthetic and consent-safe. |
| LC-016 | Sensitive drafts | Allowed with review | Policy allows non-final drafts and preparation for human/professional review; Clawix app output wrappers now persist draft, not-professional-advice, human-review, sources/gaps, and disclaimer-version labels in sensitive text exports/copies. | Generated outputs must keep mandatory labels. |
| LC-017 | Sensitive interpretation | Summary + questions | Policy allows factual summaries, questions to review, gaps, and provenance, not final advice; Dense Data systems expose `regulatedDomains` while keeping local record organization, provenance, timelines, and gap views separate from final regulated decisions; Clawix app tests verify persisted sensitive output text keeps sources/gaps and human-review language. | Search/Dense Data/agent outputs must continue to expose sources/gaps. |
| LC-018 | External sensitive actions | Explicit review | Policy, Clawix tests, Agents V1 effective access, Connector Control Plane regulated safety, MCP tool control-plane metadata, remote mesh share construction, and Search MCP action plans require sensitive external action/export/remote review; connector host approvals do not bypass regulated safety denial codes, regulated MCP tool calls are denied before protocol invocation, regulated remote shares cannot grant `execute` or `lease_secret` directly, and regulated Search results elevate even `copy` actions to review. `evaluateRegulatedAction(...)` now distinguishes `confirm` from hard `block`; CLI download/export review, SDK storage/media share/export, Notify external delivery, and Telegram/Slack/WhatsApp channel sends consume that central policy config instead of relying only on command-local legal checks. | Real provider actions remain explicit-approval work. |
| LC-019 | UI disclaimers | Contextual + remembered | Clawix legal state stores accepted versions and disclaimer policy; tests cover labels and disclaimer version. | Final visual UI smoke is `EXTERNAL PENDING` until signed-app inspection is available. |
...
| LC-026 | Audit retention | Local configurable | Privacy and Clawix persistent legal settings include local audit retention days; Clawix `LegalSafetyTests` prove the retention value is configurable and persists through the legal defaults key. | Signed app settings smoke is `EXTERNAL PENDING`. |
...
| LC-028 | Prohibited practices | Broad hard list | Regulated safety policy prohibits social scoring, harmful manipulation, sensitive biometric/emotion inference, criminal-risk profiling, final regulated decisions, emergency handling, and autonomous filings; `RELEASING.md` now requires every new sensitive collection, connector, agent, CLI route, MCP tool, Relay route, app surface, demo, or docs claim to be classified against `docs/regulated-domain-safety.md` before the release candidate can be treated as complete, and the legal verifier enforces that release-gate language. | Future implementation work must add concrete tests for each new classified route/provider. |
...
| LC-030 | Output labels | Mandatory labels | Policy and Clawix tests require draft, not professional advice, human review, sources/gaps, domain, and decision-effect labels; CLI safety human output, Dense Data regulated-domain classification, Search result actions, Search MCP audit metadata, CLI download sidecars, and Clawix sensitive text wrappers preserve regulated labels and human-readable review/sources-gaps language. | Platform share-sheet and signed export smoke are `EXTERNAL PENDING`. |
| LC-031 | Release channels | GitHub+npm+apps+web | `RELEASING.md` now separates npm package, GitHub release, website, and app/binary channel checklists; the legal verifier requires those checklist sections and exact-action approval language. Releasing docs and claim scans cover repo docs, packages, apps/binaries, website, examples, and public docs. | Channel-specific release checklists must run before each exact release action. |
| LC-032 | Versioning | Re-acceptance by version | Clawix tests cover any accepted legal document version mismatch forcing legal reacceptance across Terms, Privacy, EULA, Disclaimer, Safety, and Regulated Domains; Clawix legal gate aligns app constants and persisted acceptance defaults with legal doc versions, and ClawJS legal verifier requires all publishable legal docs to keep `Last updated: 2026-05-18`. | Material-version bump policy must be reviewed before binary release. |
| LC-033 | Sensitive exports/share | Confirmation + labels | Clawix legal tests cover sensitive action review with mandatory labels; policy requires review for export/share; CLI document/media download review now evaluates the central policy, writes approval/legal-label metadata, and persists policy decision/reason/requirement/output-label data in `.claw-legal.json` sidecars; Clawix editor/plan export wrappers and iOS editor exports preserve both machine labels and human-readable human-review/sources-gaps language through text/HTML/SVG/Markdown paths. | Final export/share implementation paths must remain guarded. |
...
- Search, Dense Data, Agents V1, Connector Control Plane, Remote/Sync, Relay,
  ...
  before final close.
- Provider, physical-device, signed-host, registry, store, website deployment,
  app/binary, notarization, TestFlight/App Store, tag, upload, and other
  exact-channel validations are separated in
  `docs/governance/legal/external-pending.md` as `EXTERNAL PENDING`, not pass.
