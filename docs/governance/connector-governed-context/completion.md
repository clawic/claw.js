...
| CGA-006 | RevenueCat API v2 is the preferred default, v1 is fallback-only, and context plus secret plus fallback plus scoped grant/approval evidence is audited. | validated | RevenueCat examples include `revenuecat_api_v2` and `revenuecat_api_v1`; schema defaults and fallback rules are tested; control-plane audit tests assert selected context refs, secret refs, default refs, fallback rule ids, scoped connector grant metadata, approval evidence, and reason codes. | Live RevenueCat API reads remain explicit-approval work. |
