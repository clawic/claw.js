...
| LC-030 | Output labels | Mandatory labels | Policy and Clawix tests require draft, not professional advice, human review, sources/gaps, domain, and decision-effect labels; CLI safety human output, Dense Data regulated-domain classification, Search result actions, Search MCP audit metadata, and Clawix sensitive text wrappers preserve regulated labels and human-readable review/sources-gaps language. | Platform share-sheet and signed export smoke are `EXTERNAL PENDING`. |
...
| LC-033 | Sensitive exports/share | Confirmation + labels | Clawix legal tests cover sensitive action review with mandatory labels; policy requires review for export/share; Clawix editor/plan export wrappers and iOS editor exports now preserve both machine labels and human-readable human-review/sources-gaps language through text/HTML/SVG/Markdown paths. | Final export/share implementation paths must remain guarded. |
