...
| LC-024 | Package disclaimers | README + CLI | Root README links legal docs and `claw safety` exposes policy/disclaimers; human-readable `safety check` output preserves disclaimer policy and mandatory labels, not only JSON. | Package README coverage must remain part of release checks. |
...
| LC-030 | Output labels | Mandatory labels | Policy and Clawix tests require draft, not professional advice, human review, sources/gaps, domain, and decision-effect labels; CLI safety human output, Search result actions, and Search MCP audit metadata preserve regulated labels. | Final output surfaces must preserve labels through export/share. |
