...
selected before being enabled, rebuild queueing is a separate choice, and the
first-run snapshot distinguishes setup-ready sources from sources that still
need a local root, cache/export, or signed host. It is deliberately separate
from Root Search and the current Clawix Mac chat search, so enabling wider
