...
The code hygiene program applies to ClawJS and Clawix. Existing debt is cleaned first by repo and category; then the changed and release lanes block new clear debt. The source conversation is `source:code-hygiene-program`, and all 33 decisions in `docs/code-hygiene-decisions.json` are binding.
