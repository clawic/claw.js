...
| DQ-002 | Initial prompt | Use medicine as an example, but generalize to any domain with dense relational data and specialized software. | First-wave registry covers health/EHR, research/CTMS, biology, labs/LIMS, legal, ERP, CRM, finance/accounting, education/LMS, HR/HRIS, manufacturing/MES, ops/ITSM, real estate/proptech, insurance, maintenance/CMMS, procurement/purchasing, warehouse/WMS, supply chain/SCM, transport/TMS, compliance/GRC, government/gov, IoT, construction, ELN, content/CMS, product/PIM/PLM, and pharma/GxP; roadmap covers energy/utilities, telecom/network ops, hospitality/PMS, agriculture/farm management, nonprofit/grants, media production, aerospace/MRO, banking/core banking, and public safety/CAD. | In progress: roadmap packs are visible but not all graduated. |
...
| DQ-006 | Catalog/CLI concern | Completeness is measured by CLI intentions such as `patient list`, saving symptoms/medications, and listing a patient’s medications. | `cli-dense-data-command.ts`, `cli-discovery.test.ts`, generated dense intents, `listClawDenseDataGapRegistryEntries`, `inspect dense-intents`, `inspect dense-gaps`. | In progress: representative first-wave routes are executable and uncovered states are queryable as dense gap registry rows; exhaustive professional intent coverage remains open. |
| DQ-007 | Catalog/CLI concern | Before closing, inspect the existing framework carefully to avoid a parallel system that ignores notes, graphs, current catalogs, or similar existing work. | Existing catalog audit, `clawDenseDataOsRegistry.existingSurfaceIntegrations`, `entity_relations` generalized instead of replaced, reuse of built-ins and `core.sqlite`, ADR 0021, `dense-data-os.test.ts`, `scripts/verify-dense-data-goal.mjs`. | In progress: structured anti-duplication guard exists for the required surfaces; final close still needs the full source-session audit and any newly discovered duplicated surfaces must be integrated, retired, or explicitly blocked. |
...
| DQ-013 | Implementation correction | ERP, legal, and similar areas must be implemented at DB/system level, not merely mentioned. | ERP/company/account/deal/invoice routes, legal case/client/evidence routes, HR employee/time-off/review routes, real-estate property/visit/offer/inspection routes, insurance policy routes, maintenance vehicle/appliance routes, procurement supplier/purchase-order/line-item routes, warehouse/inventory/stock-movement routes, supply-chain plan/item/risk routes, transport carrier/shipment/leg/rate routes, compliance control/obligation/assessment/finding routes, government agency/public-case/permit/public-filing routes, product spec/revision/requirement/BOM routes, pharma drug-product/batch-record/lot-release/adverse-event routes, content brand/destination/campaign/entry/revision/variant/approval/publication routes, IoT thing/device/reading/command routes, construction project/site/RFI/change-order routes, ELN lab-notebook/notebook-entry/protocol-run/experiment-observation routes, ops service/incident routes, first-wave built-ins, fixture records. | In progress: representative DB/system implementation exists; richer pack workflows remain. |
