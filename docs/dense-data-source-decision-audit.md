...
| DQ-013 | Implementation correction | ERP, legal, and similar areas must be implemented at DB/system level, not merely mentioned. | ERP/company/account/deal/invoice routes, legal case/client/evidence routes, HR employee/time-off/review routes, real-estate property/visit/offer/inspection routes, insurance policy routes, maintenance vehicle/appliance routes, procurement supplier/purchase-order/line-item routes, warehouse/inventory/stock-movement routes, supply-chain plan/item/risk routes, transport carrier/shipment/leg/rate routes, compliance control/obligation/assessment/finding routes, IoT thing/device/reading/command routes, construction project/site/RFI/change-order routes, ELN lab-notebook/notebook-entry/protocol-run/experiment-observation routes, ops service/incident routes, first-wave built-ins, fixture records. | In progress: representative DB/system implementation exists; richer pack workflows remain. |
