...
| `unused_files` | Entrypoints configurados primero | partially implemented | Knip config declares explicit entry/project scope, includes tool configs/scripts/seeds/public JS assets, and maps top-level app/module workspaces; full ClawJS file cleanup pending. |
| `dependency_policy` | Normalizar por workspace | partially implemented | Removed clear unused ClawJS workspace dependencies (`chokidar`, profile `@noble/hashes`, reviewed app/database direct dependencies), moved root VitePress/Vue ownership to `website`, declared direct root `tsx` usage, calibrated the Vitest coverage provider, and mapped top-level app/module package manifests with explicit entry/project shapes; remaining dependency findings stay report-only. |
