...
| 4 | `v1_depth` | partial | Contracts, core gates, CLI/SDK/inspect, service API envelope, and hermetic POST handler exist; native host/UI validation and final Q/A acceptance still remain. |
...
| 51 | `agent_ui_scope` | partial | ADR defines the full UI scope and the framework now exposes a shared `control-panel` projection; native Clawix UI implementation remains outside this first framework slice. |
...
| 84 | `implementation_slice` | partial | Model, gates, CLI/SDK/inspect, service API handler, control panel, privacy plan, and legacy board migration are implemented; native host/UI validation and final private decision audit remain. |
