...
| RQ-006 | `remote_secrets_model` | Secrets sync as references only; operations require brokered leases and audit. | Secret policy forbids plaintext replication; remote secret lease contracts return no plaintext; `claw gateway secret-lease` records signed expiring lease metadata for secret refs only. |
...
| RQ-020 | `mesh_collaboration_scope` | Mesh collaboration needs invitation, scoped share, and revocation primitives. | `mesh.resourceShare` and `nodes pair|trust|revoke` are required dry-run/signed-host surfaces; `claw nodes invite|share|revoke --state-dir <dir> --record true` records durable local proposals/revocations without granting trust. |
