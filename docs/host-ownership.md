...
`.claw/`, and framework code must not add new `.clawjs` readers or migrations
unless an ADR explicitly grants a bounded removal exception.
...
identifier against the enclosing Clawix bundle identifier and checks the caller
TeamIdentifier against the signed service before issuing assertions. Password
unlock uses password + Secret Key; local biometric unlock uses native LAContext
