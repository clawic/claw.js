...
| MC-010 | Legacy reset | Commander is not a compatibility constraint; direct native uses must migrate or be registered. | ADR 0023, host permission guard, `docs/mac-native-legacy-audit.md`. | Static guard self-test, Mac goal verifier, legacy audit review. | Migrate or retire each `MNL-*` row before final closure unless explicitly retained under non-Mac-Control ownership. |
