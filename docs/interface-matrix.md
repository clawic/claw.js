...
| Design resources | Clawix Design styles, templates, references, and editor | design/resource registry APIs | `claw design list|upsert|delete` | `optional` local service contract | MCP resources for design assets | `local-only` unless explicitly synced | framework workspace design records |
...
### Remote Gateway And Sync Interfaces
...
| Nodes and mesh collaboration | `claw nodes list|pair|trust|revoke|invite|share|heartbeat` | `/v1/nodes`, `/v1/nodes/pair`, `/v1/nodes/trust`, `/v1/nodes/revoke`, `/v1/mesh/invitations`, `/v1/mesh/invitations/accept`, `/v1/mesh/shares`, `/v1/mesh/revocations` | Explicit pairing, trust, invitations, scoped shares, and revocation; physical trust remains external pending until approved. | node trust decisions, mesh invitations, shares, revocations |
...
| Provider enabled settings | framework provider settings config | `claw providers settings list|set|delete` | `-` |
...
| Design resource registry | design/resource registry APIs | `claw design list|upsert|delete` | `-` |
