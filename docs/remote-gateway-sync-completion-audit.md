...
| RQ-008 | `remote_api_shape` | implemented | `RemoteRouteContractCatalog`, `claw remote contracts`, and Relay route contracts forbid parallel APIs and bind local refs to remote entrypoints. | Expand per-domain remote-safe conformance as each domain graduates. |
...
| RQ-012 | `first_vertical_slice` | external_pending | Route contracts cover chat, search, secret broker operation, sync, headless host, and multi-tenant agent service. | Real provider/device end-to-end validation remains `provider_device_e2e` `EXTERNAL PENDING`. |
...
| --- | --- | --- | --- |
| SOURCE-REREAD-001 | in_progress | Source decisions are enumerated as `RQ-001` through `RQ-022` and guarded by `scripts/verify-remote-sync-goal.mjs`. | Before closing the private goal, re-read the source JSONL one by one and confirm every decision-bearing user answer maps to these rows or add the missing row. |
...
| Relay HTTP routes | `npx vitest run --config vitest.config.ts relay/src/server/remote-sync-routes.test.ts` |
