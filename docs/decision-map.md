...
| Package/live connector validation installs the candidate package before any live broker command can receive lease context. | [Integration QA Lab](./integration-qa-lab.md), `scripts/verify-integration-package-live-harness.mjs` | `npm run test:package-live` packs and installs `@clawjs/integrations` into a temporary app, rejects raw Telegram token env vars, and reports live/Docker prerequisites as `EXTERNAL PENDING` when not explicitly enabled. |
