...
| User-facing structured records belong in `core.sqlite`; high-churn runtime/search/blob state uses sidecars; plaintext secrets never live in the main database. | [Data Storage Boundary](./data-storage-boundary.md), [Database](./database.md), [Secrets](./secrets.md) | `scripts/docs-alignment-check.mjs` requires `core.sqlite`, sidecar names, and plaintext secret language. Service tests cover storage behavior for individual packages. |
