...
| Built-in collections follow the canonical data catalog: broad human-recognizable coverage, standardized names, sparse fields, semantic relations, metadata evidence, and conservative evolution. | [Canonical Data Catalog](./canonical-data-catalog.md), [ADR 0005: canonical data catalog](./adr/0005-canonical-data-catalog.md), [Database](./database.md), root `CONSTITUTION.md`, `packages/clawjs-core/src/builtins/_types.ts` | Built-in catalog tests grandfather the existing catalog but require metadata for new collections, validate aliases, validate optional relation semantics, and keep CRUD smoke coverage for all built-ins. |
