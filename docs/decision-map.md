...
| Durable paths, databases, sidecars, host operational paths, legacy read-only paths, external read-only sources, preference keys, app/browser storage keys, tables, columns, indexes, caches, fixtures, and persistent temps are registered through typed builders and inspected through `claw inspect`. | [ADR 0004: persistent surface registry and inspection](./adr/0004-persistent-surface-registry-and-inspection.md), [Persistent Surface](./persistent-surface.md), `packages/clawjs-core/src/surface-registry.ts` | Core tests cover registry lookup/tree behavior; CLI tests cover `claw inspect tree`, `show`, generated Markdown/Mermaid output, and static manifest fusion. `scripts/persistent-surface-guard.mjs` blocks unregistered durable names. |
