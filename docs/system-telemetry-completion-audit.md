...
| STA-014 | Re-read source decisions one by one before any completion claim. | active-closure-gate | `docs/system-telemetry-source-qa-review.json`, private audit alias, external manifest `sourceQaReview`, and verifier checks. | Final source reread must be repeated before `update_goal complete`. |
