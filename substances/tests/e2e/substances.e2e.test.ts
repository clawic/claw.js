...
import { buildSubstancesApp, SubstancesClient } from "@clawjs/substances";

test("substances CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "substances-e2e-"));
  const dbPath = path.join(tmpDir, "substances.sqlite");
  ...
  try {
    ...
    const client = new SubstancesClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "substances");
