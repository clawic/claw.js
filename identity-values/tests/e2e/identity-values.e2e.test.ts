...
import { buildIdentityValuesApp, IdentityValuesClient } from "@clawjs/identity-values";
...
test("identity-values CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new IdentityValuesClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "identity-values");
