...
import { buildWhatsappApp } from "../../src/server/app.ts";
...
async function spinUp() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "whatsapp-e2e-"));
  const built = buildWhatsappApp({ config: { host: "127.0.0.1", port: 0, dataDir: tmpDir, dbPath: path.join(tmpDir, "whatsapp.sqlite"), sharedSecret: SECRET } });
  const client = new ChannelApiClient({ channel: "whatsapp", baseUrl: "http://whatsapp.test", token: SECRET, fetchImpl: injectFetch(built.app) });
  return { client, tmpDir, close: async () => { await built.app.close(); fs.rmSync(tmpDir, { recursive: true, force: true }); } };
}

test("whatsapp: account CRUD round-trip", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary", credentialsRef: "vault://whatsapp/primary" });
...
test("whatsapp: routing assign + resolve + remove", async () => {
  const ctx = await spinUp();
  try {
...
test("whatsapp: send via stub transport records outbound message", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary" });
    const msg = await ctx.client.sendMessage({ accountId: account.id, targetId: "target-1", content: "hola desde whatsapp" });
...
test("whatsapp: inbound webhook lands in messages table", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary" });
...
test("whatsapp: send to unknown account returns 404", async () => {
  const ctx = await spinUp();
  try {
