 ...
 *
 * Generates rich, realistic synthetic mock data for the demo showcase mode.
 * All people, messages, emails, tokens, costs, and incidents are synthetic placeholders.
...
function generateInbox() {
  return [
    ...
    { id: genId(), channel: "email", from: "alerts@example.com", subject: "Cloud Cost Alert: 85% of monthly budget", preview: "Your synthetic cloud account has reached 85% of its monthly budget.", content: "Your synthetic cloud account project-demo has reached 85% of the monthly budget limit.\n\nReview the current spend breakdown and investigate the highest-cost services before the next deploy window.\n\nTop cost drivers:\n- Compute\n- Database\n- Object storage\n\nReview your costs at https://billing.example.com", read: true, timestamp: nowMinus(420), threadId: "thread-cloud" },
    ...
    { id: genId(), channel: "email", from: "noreply@example.com", subject: "[clawjs/demo] PR #287: Add caching layer", preview: "Jordan Lee requested your review on PR #287", content: "Jordan Lee requested your review on pull request #287: Add caching layer\n\n+342 -18 across 8 files\n\nKey changes:\n- New CacheManager class\n- Redis integration for query caching\n- TTL-based invalidation\n- Fallback to direct DB on cache miss", read: true, timestamp: nowMinus(360), threadId: "thread-pr-287" },
  ];
}
...
function generateE2EIntegrations() {
  return {
    ...
    email: {
      ...
      backend: "mock",
      enabled: true,
      accounts: [{ id: "inbox", email: "alex@example.com", displayName: "Alex Taylor", default: true }],
