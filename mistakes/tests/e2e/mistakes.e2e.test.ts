...
import { buildMistakesApp, MistakesClient } from "@clawjs/mistakes";

test("mistakes CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mistakes-e2e-"));
  const dbPath = path.join(tmpDir, "mistakes.sqlite");
  ...
  try {
    ...
    const client = new MistakesClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "mistakes");
