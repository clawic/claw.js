...
test("native folder/doc/sheet/slide flows persist and revision conflicts are enforced", async () => {
  ...
  const folderResponse = await authFetch(clawApiPath("items"), {
    method: "POST",
  ...
  const docResponse = await authFetch(clawApiPath("items"), {
    method: "POST",
  ...
  const docDetailResponse = await authFetch(clawApiPath(`items/${doc.id}`));
  ...
  const saveResponse = await authFetch(clawApiPath(`items/${doc.id}/content`), {
    method: "POST",
    body: JSON.stringify({
  ...
  const conflictResponse = await authFetch(clawApiPath(`items/${doc.id}/content`), {
    method: "POST",
    body: JSON.stringify({
      baseRevisionId: docDetail.currentRevisionId,
      content: docDetail.content,
...
test("uploads, shares, download, search, trash, restore, copy, and export work", async () => {
  ...
  const uploadResponse = await authFetch(clawApiPath("uploads"), { method: "POST", body: form });
  ...
  const shareResponse = await authFetch(clawApiPath(`items/${uploaded.id}/shares`), {
    method: "POST",
  ...
  const downloadResponse = await authFetch(clawApiPath(`items/${uploaded.id}/download`));
  assert.equal(downloadResponse.status, 200);
  assert.equal(Buffer.from(await downloadResponse.arrayBuffer()).toString("utf8"), "Quarterly notes for launch review");
  const recentAfterDownload = await (await authFetch(clawApiPath("items?view=recent"))).json() as { items: Array<{ id: string }> };
  assert.ok(recentAfterDownload.items.some((item) => item.id === uploaded.id));

  const searchResponse = await authFetch(clawApiPath("items?view=my-drive&q=quarterly"));
  const search = await searchResponse.json() as { items: Array<{ id: string }> };
  assert.ok(search.items.some((item) => item.id === uploaded.id));

  const copyResponse = await authFetch(clawApiPath(`items/${uploaded.id}/copy`), {
    method: "POST",
    body: JSON.stringify({}),
  });
  ...
  const trashResponse = await authFetch(clawApiPath(`items/${uploaded.id}/trash`), { method: "POST" });
  assert.equal(trashResponse.status, 200);
  const trashList = await (await authFetch(clawApiPath("items?view=trash"))).json() as { items: Array<{ id: string }> };
  assert.ok(trashList.items.some((item) => item.id === uploaded.id));

  const restoreResponse = await authFetch(clawApiPath(`items/${uploaded.id}/restore`), { method: "POST" });
  ...
  const exportResponse = await authFetch(clawApiPath(`items/${doc.id}/export?format=pdf`));
...
test("agent share revoke writes one audit event", async () => {
  ...
  const revokeResponse = await authFetch(clawApiPath(`items/${uploaded.id}/shares/${share.record.id}/revoke`), {
    method: "POST",
  });
  assert.equal(revokeResponse.status, 200);

  const auditResponse = await authFetch(clawApiPath(`audit?itemId=${uploaded.id}&limit=20`));
...
test("scoped tokens can read lists after creation", async () => {
  ...
  const tokenResponse = await authFetch(clawApiPath("tokens"), {
    method: "POST",
  ...
  const revokeResponse = await authFetch(clawApiPath(`tokens/${token.record.id}/revoke`), { method: "POST" });
  assert.equal(revokeResponse.status, 200);
});
