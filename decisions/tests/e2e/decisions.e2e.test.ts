...
import { buildDecisionsApp, DecisionsClient } from "@clawjs/decisions";

test("decisions CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "decisions-e2e-"));
  const dbPath = path.join(tmpDir, "decisions.sqlite");
  ...
  try {
    ...
    const client = new DecisionsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "decisions");
