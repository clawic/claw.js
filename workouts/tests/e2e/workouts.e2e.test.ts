...
import { buildWorkoutsApp, WorkoutsClient } from "@clawjs/workouts";
...
test("workouts CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new WorkoutsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "workouts");
