...
import { buildVolunteeringApp, VolunteeringClient } from "@clawjs/volunteering";

test("volunteering CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "volunteering-e2e-"));
  const dbPath = path.join(tmpDir, "volunteering.sqlite");
  ...
  try {
    ...
    const client = new VolunteeringClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "volunteering");
