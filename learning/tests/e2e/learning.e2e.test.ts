...
import { buildLearningApp, LearningClient } from "@clawjs/learning";

test("learning CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "learning-e2e-"));
  const dbPath = path.join(tmpDir, "learning.sqlite");
  ...
  try {
    ...
    const client = new LearningClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "learning");
