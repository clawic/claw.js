...
import { buildNetworkingApp, NetworkingClient } from "@clawjs/networking";

test("networking CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "networking-e2e-"));
  const dbPath = path.join(tmpDir, "networking.sqlite");
  ...
  try {
    ...
    const client = new NetworkingClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "networking");
