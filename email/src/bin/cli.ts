...
if (!group || argv.includes("--help")) {
  process.stdout.write([
    "Usage: email <command> [options]",
    "  email serve [--port N] [--secret TOKEN]",
    "  email accounts add --name N [--credentials-ref SECRET]",
    "  email accounts list",
    "  email accounts remove --id ID",
    "  email routing assign --account ID --target T --agent A",
    "  email routing list [--account ID] [--agent A]",
    "  email send --account ID --target T --content TEXT",
    "  email messages [--account ID] [--target T] [--direction inbound|outbound]",
  ].join("\n") + "\n");
  process.exit(0);
}
async function main() {
  if (group === "serve") {
    const { app } = buildEmailApp({ config: { ...(flags.port ? { port: Number(flags.port) } : {}), ...(flags.secret ? { sharedSecret: flags.secret } : {}) } });
    const addr = await app.listen({ host: flags.host ?? "127.0.0.1", port: flags.port ? Number(flags.port) : 4704 });
    process.stdout.write(`${addr}\n`); return;
  }
  const baseUrl = flags.url ?? "http://127.0.0.1:4704"; const token = flags.token ?? flags.secret ?? "";
  const client = new ChannelApiClient({ channel: "email", baseUrl, token });
