...
import { buildDatingApp, DatingClient } from "@clawjs/dating";

test("dating CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "dating-e2e-"));
  const dbPath = path.join(tmpDir, "dating.sqlite");
  ...
  try {
    ...
    const client = new DatingClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "dating");
