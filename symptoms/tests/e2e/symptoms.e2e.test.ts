...
import { buildSymptomsApp, SymptomsClient } from "@clawjs/symptoms";

test("symptoms CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "symptoms-e2e-"));
  const dbPath = path.join(tmpDir, "symptoms.sqlite");
  ...
  try {
    ...
    const client = new SymptomsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "symptoms");
