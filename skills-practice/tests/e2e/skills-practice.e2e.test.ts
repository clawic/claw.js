...
import { buildSkillsPracticeApp, SkillsPracticeClient } from "@clawjs/skills-practice";

test("skills-practice CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "skills-practice-e2e-"));
  const dbPath = path.join(tmpDir, "skills-practice.sqlite");
  ...
  try {
    ...
    const client = new SkillsPracticeClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "skills-practice");
