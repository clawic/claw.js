...
test("runtime adapters expose capability-driven details without OpenClaw regressions", async ({ page, request }) => {
  ...
  await expect(page.getByTestId("adapter-hermes-session-transport")).toContainText("hybrid -> cli");
  ...
  await expect(page.getByTestId("adapter-nanobot-session-transport")).toContainText("hybrid -> cli");
