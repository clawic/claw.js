...
import { buildDreamsApp, DreamsClient } from "@clawjs/dreams";

test("dreams CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "dreams-e2e-"));
  const dbPath = path.join(tmpDir, "dreams.sqlite");
  ...
  try {
    ...
    const client = new DreamsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "dreams");
