...
import { buildPersonalityTestsApp, PersonalityTestsClient } from "@clawjs/personality-tests";

test("personality-tests CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "personality-tests-e2e-"));
  const dbPath = path.join(tmpDir, "personality-tests.sqlite");
  ...
  try {
    ...
    const client = new PersonalityTestsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "personality-tests");
