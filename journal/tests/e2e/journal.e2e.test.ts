...
import { buildJournalApp, JournalClient } from "@clawjs/journal";

test("journal CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "journal-e2e-"));
  const dbPath = path.join(tmpDir, "journal.sqlite");
  ...
  try {
    ...
    const client = new JournalClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "journal");
