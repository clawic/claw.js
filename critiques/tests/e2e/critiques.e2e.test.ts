...
import { buildCritiquesApp, CritiquesClient } from "@clawjs/critiques";
...
test("critiques CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new CritiquesClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "critiques");
