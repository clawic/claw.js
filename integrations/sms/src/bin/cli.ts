...
if (!group || argv.includes("--help")) {
  process.stdout.write([
    ...
    "  sms serve [--port N] [--secret TOKEN]",
    "  sms accounts add --name N [--credentials-ref SECRET]",
    "  sms accounts list",
    "  sms accounts remove --id ID",
    "  sms routing assign --account ID --target T --agent A",
    "  sms routing list [--account ID] [--agent A]",
    "  sms send --account ID --target T --content TEXT",
    "  sms messages [--account ID] [--target T] [--direction inbound|outbound]",
  ].join("\n") + "\n");
  process.exit(0);
}
async function main() {
  if (group === "serve") {
    const { app } = buildSmsApp({ config: { ...(flags.port ? { port: Number(flags.port) } : {}), ...(flags.secret ? { sharedSecret: flags.secret } : {}) } });
    const addr = await app.listen({ host: flags.host ?? "127.0.0.1", port: flags.port ? Number(flags.port) : 4707 });
    process.stdout.write(`${addr}\n`); return;
  }
  const baseUrl = flags.url ?? "http://127.0.0.1:4707"; const token = flags.token ?? flags.secret ?? "";
  const client = new ChannelApiClient({ channel: "sms", baseUrl, token });
