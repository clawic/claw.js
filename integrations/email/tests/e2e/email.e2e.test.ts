...
import { buildEmailApp } from "../../src/server/app.ts";

const SECRET = "email-e2e-secret";
...
async function spinUp() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "email-e2e-"));
  const built = buildEmailApp({ config: { host: "127.0.0.1", port: 0, dataDir: tmpDir, dbPath: path.join(tmpDir, "clawjs.sqlite"), sharedSecret: SECRET } });
  const client = new ChannelApiClient({ channel: "email", baseUrl: "http://email.test", token: SECRET, fetchImpl: injectFetch(built.app) });
  return { client, tmpDir, close: async () => { await built.app.close(); fs.rmSync(tmpDir, { recursive: true, force: true }); } };
}

test("email: account CRUD round-trip", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary", credentialsRef: "vault://email/primary" });
    assert.equal(account.channel, "email");
...
test("email: routing assign + resolve + remove", async () => {
  const ctx = await spinUp();
  try {
...
test("email: send via stub transport records outbound message", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary" });
    const msg = await ctx.client.sendMessage({ accountId: account.id, targetId: "target-1", content: "hola desde email" });
...
test("email: inbound webhook lands in messages table", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary" });
...
test("email: send to unknown account returns 404", async () => {
  const ctx = await spinUp();
  try {
