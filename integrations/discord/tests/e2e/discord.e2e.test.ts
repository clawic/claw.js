...
import { buildDiscordApp } from "../../src/server/app.ts";

const SECRET = "discord-e2e-secret";
...
async function spinUp() {
  ...
  const built = buildDiscordApp({ config: { host: "127.0.0.1", port: 0, dataDir: tmpDir, dbPath: path.join(tmpDir, "core.sqlite"), sharedSecret: SECRET } });
...
test("discord: account CRUD round-trip", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary", credentialsRef: "vault://discord/primary" });
...
test("discord: routing assign + resolve + remove", async () => {
  const ctx = await spinUp();
  try {
...
test("discord: send via stub transport records outbound message", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary" });
    const msg = await ctx.client.sendMessage({ accountId: account.id, targetId: "target-1", content: "hola desde discord" });
...
test("discord: inbound webhook lands in messages table", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary" });
...
test("discord: send to unknown account returns 404", async () => {
  const ctx = await spinUp();
  try {
