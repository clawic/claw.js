...
if (!group || argv.includes("--help")) {
  process.stdout.write([
    "Usage: whatsapp <command> [options]",
    "  whatsapp serve [--port N] [--secret TOKEN]",
    "  whatsapp accounts add --name N [--credentials-ref SECRET]",
    "  whatsapp accounts list",
    "  whatsapp accounts remove --id ID",
    "  whatsapp routing assign --account ID --target T --agent A",
    "  whatsapp routing list [--account ID] [--agent A]",
    "  whatsapp send --account ID --target T --content TEXT",
    "  whatsapp messages [--account ID] [--target T] [--direction inbound|outbound]",
  ].join("\n") + "\n");
  process.exit(0);
}
async function main() {
  if (group === "serve") {
    const { app } = buildWhatsappApp({ config: { ...(flags.port ? { port: Number(flags.port) } : {}), ...(flags.secret ? { sharedSecret: flags.secret } : {}) } });
    const addr = await app.listen({ host: flags.host ?? "127.0.0.1", port: flags.port ? Number(flags.port) : 4705 });
    process.stdout.write(`${addr}\n`); return;
  }
  const baseUrl = flags.url ?? "http://127.0.0.1:4705"; const token = flags.token ?? flags.secret ?? "";
  const client = new ChannelApiClient({ channel: "whatsapp", baseUrl, token });
