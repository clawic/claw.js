...
if (!group || argv.includes("--help")) {
  process.stdout.write([
    "Usage: teams <command> [options]",
    "  teams serve [--port N] [--secret TOKEN]",
    "  teams accounts add --name N [--credentials-ref SECRET]",
    "  teams accounts list",
    "  teams accounts remove --id ID",
    "  teams routing assign --account ID --target T --agent A",
    "  teams routing list [--account ID] [--agent A]",
    "  teams send --account ID --target T --content TEXT",
    "  teams messages [--account ID] [--target T] [--direction inbound|outbound]",
  ].join("\n") + "\n");
  process.exit(0);
}
async function main() {
  if (group === "serve") {
    const { app } = buildTeamsApp({ config: { ...(flags.port ? { port: Number(flags.port) } : {}), ...(flags.secret ? { sharedSecret: flags.secret } : {}) } });
    const addr = await app.listen({ host: flags.host ?? "127.0.0.1", port: flags.port ? Number(flags.port) : 4709 });
    process.stdout.write(`${addr}\n`); return;
  }
  const baseUrl = flags.url ?? "http://127.0.0.1:4709"; const token = flags.token ?? flags.secret ?? "";
  const client = new ChannelApiClient({ channel: "teams", baseUrl, token });
