...
if (!group || argv.includes("--help")) {
  process.stdout.write([
    "Usage: signal <command> [options]",
    "  signal serve [--port N] [--secret TOKEN]",
    "  signal accounts add --name N [--credentials-ref SECRET]",
    "  signal accounts list",
    "  signal accounts remove --id ID",
    "  signal routing assign --account ID --target T --agent A",
    "  signal routing list [--account ID] [--agent A]",
    "  signal send --account ID --target T --content TEXT",
    "  signal messages [--account ID] [--target T] [--direction inbound|outbound]",
  ].join("\n") + "\n");
  process.exit(0);
}
async function main() {
  if (group === "serve") {
    const { app } = buildSignalApp({ config: { ...(flags.port ? { port: Number(flags.port) } : {}), ...(flags.secret ? { sharedSecret: flags.secret } : {}) } });
    const addr = await app.listen({ host: flags.host ?? "127.0.0.1", port: flags.port ? Number(flags.port) : 4706 });
    process.stdout.write(`${addr}\n`); return;
  }
  const baseUrl = flags.url ?? "http://127.0.0.1:4706"; const token = flags.token ?? flags.secret ?? "";
  const client = new ChannelApiClient({ channel: "signal", baseUrl, token });
