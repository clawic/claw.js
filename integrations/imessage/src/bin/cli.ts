...
if (!group || argv.includes("--help")) {
  process.stdout.write([
    ...
    "  imessage serve [--port N] [--secret TOKEN]",
    "  imessage accounts add --name N [--credentials-ref SECRET]",
    "  imessage accounts list",
    "  imessage accounts remove --id ID",
    "  imessage routing assign --account ID --target T --agent A",
    "  imessage routing list [--account ID] [--agent A]",
    "  imessage send --account ID --target T --content TEXT",
    "  imessage messages [--account ID] [--target T] [--direction inbound|outbound]",
  ].join("\n") + "\n");
  process.exit(0);
}
async function main() {
  if (group === "serve") {
    const { app } = buildImessageApp({ config: { ...(flags.port ? { port: Number(flags.port) } : {}), ...(flags.secret ? { sharedSecret: flags.secret } : {}) } });
    const addr = await app.listen({ host: flags.host ?? "127.0.0.1", port: flags.port ? Number(flags.port) : 4703 });
    process.stdout.write(`${addr}\n`); return;
  }
  const baseUrl = flags.url ?? "http://127.0.0.1:4703"; const token = flags.token ?? flags.secret ?? "";
  const client = new ChannelApiClient({ channel: "imessage", baseUrl, token });
