...
import { buildImessageApp } from "../../src/server/app.ts";

const SECRET = "imessage-e2e-secret";
...
async function spinUp() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "imessage-e2e-"));
  const built = buildImessageApp({ config: { host: "127.0.0.1", port: 0, dataDir: tmpDir, dbPath: path.join(tmpDir, "imessage.sqlite"), sharedSecret: SECRET } });
  const client = new ChannelApiClient({ channel: "imessage", baseUrl: "http://imessage.test", token: SECRET, fetchImpl: injectFetch(built.app) });
  return { client, tmpDir, close: async () => { await built.app.close(); fs.rmSync(tmpDir, { recursive: true, force: true }); } };
}

test("imessage: account CRUD round-trip", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary", credentialsRef: "vault://imessage/primary" });
    assert.equal(account.channel, "imessage");
...
test("imessage: routing assign + resolve + remove", async () => {
  const ctx = await spinUp();
  try {
...
test("imessage: send via stub transport records outbound message", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary" });
    const msg = await ctx.client.sendMessage({ accountId: account.id, targetId: "target-1", content: "hola desde imessage" });
...
test("imessage: inbound webhook lands in messages table", async () => {
  const ctx = await spinUp();
  try {
    const account = await ctx.client.createAccount({ name: "primary" });
...
test("imessage: send to unknown account returns 404", async () => {
  const ctx = await spinUp();
  try {
