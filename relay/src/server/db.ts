...
import { RELAY_SCHEMA_COLUMN_MIGRATIONS, RELAY_SCHEMA_SQL } from "./db-schema.ts";
import { SEEDED_PASSWORD_HASHES, hashSecret, now, parseJsonArray, parseJsonObject, type MembershipRow, type TableColumnRow, type UserRow } from "./db-helpers.ts";
...
export class RelayDatabase {
  ...
  private init(): void {
    this.sqlite.exec(RELAY_SCHEMA_SQL);
    for (const column of RELAY_SCHEMA_COLUMN_MIGRATIONS) {
      this.ensureColumn(column.tableName, column.columnName, column.columnDefinition);
    }
  }
