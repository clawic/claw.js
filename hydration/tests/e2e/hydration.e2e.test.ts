...
import { buildHydrationApp, HydrationClient } from "@clawjs/hydration";

test("hydration CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "hydration-e2e-"));
  const dbPath = path.join(tmpDir, "hydration.sqlite");
  ...
  try {
    ...
    const client = new HydrationClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "hydration");
