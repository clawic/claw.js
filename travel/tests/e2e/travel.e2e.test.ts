...
import { buildTravelApp, TravelClient } from "@clawjs/travel";

test("travel CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "travel-e2e-"));
  const dbPath = path.join(tmpDir, "travel.sqlite");
  ...
  try {
    ...
    const client = new TravelClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "travel");
