...
import { buildTimeTrackingApp, TimeTrackingClient } from "@clawjs/time-tracking";

test("time-tracking CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "time-tracking-e2e-"));
  const dbPath = path.join(tmpDir, "time-tracking.sqlite");
  ...
  try {
    ...
    const client = new TimeTrackingClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "time-tracking");
