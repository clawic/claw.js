...
import { buildCycleApp, CycleClient } from "@clawjs/cycle";

test("cycle CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "cycle-e2e-"));
  const dbPath = path.join(tmpDir, "cycle.sqlite");
  ...
  try {
    ...
    const client = new CycleClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "cycle");
