...
import { buildBookmarksApp, BookmarksClient } from "@clawjs/bookmarks";
...
test("bookmarks CRUD smoke", async () => {
  ...
  try {
    ...
    const client = new BookmarksClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "bookmarks");
