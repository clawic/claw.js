...
export async function startServer(options: BuildAppOptions & { statusFile?: string } = {}) {
  ...
  if (options.statusFile) {
    ...
    fs.writeFileSync(
      options.statusFile,
      JSON.stringify({ ok: true, service: "badger", host: built.config.host, port: built.config.port, pid: process.pid, startedAt: new Date().toISOString() }, null, 2),
      "utf8",
    );
  }
  return built;
...
if (isDirectInvocation) {
  ...
  process.stdout.write(`badger listening on http://${built.config.host}:${built.config.port}\n`);
