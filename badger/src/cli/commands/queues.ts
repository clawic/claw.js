...
export async function queues(ctx: CliContext): Promise<number> {
  ...
  if (!command || command === "list") {
    out(ctx, await ctx.client.get(`/v1/ws/${ws}/queues`));
    return 0;
  }
  if (command === "create") {
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/queues`, {
  ...
  if (command === "pause") {
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/queues/${sub}/pause`));
    return 0;
  }
  if (command === "resume") {
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/queues/${sub}/resume`));
    return 0;
  }
  if (command === "slots" && sub === "add") {
    ...
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/queues/${queueId}/slots`, {
  ...
  if (command === "attach") {
    ...
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/queues/${queueId}/accounts`, { channel_account_id: account }));
    return 0;
  }
  if (command === "enqueue") {
    const queueId = ctx.args.flags.queue;
    ...
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/queues/${queueId}/entries`, { post_id: postId }));
    return 0;
  }
...
export async function calendar(ctx: CliContext): Promise<number> {
  ...
  out(ctx, await ctx.client.get(`/v1/ws/${ws}/posts?from=${encodeURIComponent(fromDate)}&to=${encodeURIComponent(toDate)}`));
  return 0;
}
...
export async function blackouts(ctx: CliContext): Promise<number> {
  ...
  if (command === "add") {
    out(ctx, await ctx.client.post(`/v1/ws/${ws}/blackouts`, {
