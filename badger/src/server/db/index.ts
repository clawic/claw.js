...
export class PrefixedBadgerDatabase {
  constructor(private readonly db: NativeDB) {}

  prepare(sql: string): any {
    return this.db.prepare(rewriteBadgerSql(sql));
  }

  exec(sql: string) {
    return this.db.exec(rewriteBadgerSql(sql));
  }
...
export type DB = PrefixedBadgerDatabase;

export function openDatabase(dbPath: string): DB {
  fs.mkdirSync(path.dirname(dbPath), { recursive: true });
  const db = new PrefixedBadgerDatabase(new Database(dbPath));
...
function rewriteBadgerSql(sql: string): string {
  let rewritten = sql;
