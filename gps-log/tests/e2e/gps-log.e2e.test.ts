...
import { buildGpsLogApp, GpsLogClient } from "@clawjs/gps-log";

test("gps-log CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "gps-log-e2e-"));
  const dbPath = path.join(tmpDir, "gps-log.sqlite");
  ...
  try {
    ...
    const client = new GpsLogClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "gps-log");
