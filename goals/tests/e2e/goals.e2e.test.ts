...
import { buildGoalsApp, GoalsClient } from "@clawjs/goals";

test("goals CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "goals-e2e-"));
  const dbPath = path.join(tmpDir, "goals.sqlite");
  ...
  try {
    ...
    const client = new GoalsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "goals");
