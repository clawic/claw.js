...
import { buildDonationsApp, DonationsClient } from "@clawjs/donations";

test("donations CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "donations-e2e-"));
  const dbPath = path.join(tmpDir, "donations.sqlite");
  ...
  try {
    ...
    const client = new DonationsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "donations");
