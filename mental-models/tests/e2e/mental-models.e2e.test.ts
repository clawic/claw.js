...
import { buildMentalModelsApp, MentalModelsClient } from "@clawjs/mental-models";

test("mental-models CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mental-models-e2e-"));
  const dbPath = path.join(tmpDir, "mental-models.sqlite");
  ...
  try {
    ...
    const client = new MentalModelsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "mental-models");
