...
import { buildContentApp } from "../../src/server/app.ts";
import { buildTimeApp } from "../../../time/src/server/app.ts";

export async function startTimeServer(prefix = "content-time") {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), `${prefix}-`));
  const { app } = buildTimeApp({
    config: {
      host: "127.0.0.1",
      port: 0,
  ...
  const port = typeof address === "object" && address ? address.port : 4730;
  return {
    rootDir,
...
export async function startContentServer(prefix = "content-e2e") {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), `${prefix}-`));
  const time = await startTimeServer(`${prefix}-time`);
  const { app } = buildContentApp({
    config: {
      ...
      timeBaseUrl: time.baseUrl,
    },
  });
  await app.listen({ host: "127.0.0.1", port: 0 });
  const address = app.server.address();
  const port = typeof address === "object" && address ? address.port : 4650;
