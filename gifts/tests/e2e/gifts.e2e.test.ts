...
import { buildGiftsApp, GiftsClient } from "@clawjs/gifts";

test("gifts CRUD smoke", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "gifts-e2e-"));
  const dbPath = path.join(tmpDir, "gifts.sqlite");
  ...
  try {
    ...
    const client = new GiftsClient({ baseUrl, token: sharedSecret });

    const health = await client.health();
    assert.equal(health.ok, true);
    assert.equal(health.service, "gifts");
